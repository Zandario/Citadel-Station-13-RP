...
obj/item/integrated_circuit/converter/concatenator/do_work()
	...
	if(spamprotection >= max_string_length*2 && assembly)
...
obj/item/integrated_circuit/converter/findstring
	...
	activators = list(
		"search" =		 IC_PINTYPE_PULSE_IN,
		"after search" = IC_PINTYPE_PULSE_OUT,
...
obj/item/integrated_circuit/converter/exploders/do_work()
	...
	if(delimiter == null)
		set_pin_data(IC_OUTPUT, 1, text2charlist(string))
	else
		set_pin_data(IC_OUTPUT, 1, splittext(string, delimiter))
	push_data()
	activate_pin(2)
