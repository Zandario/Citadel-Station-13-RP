...
/obj/item/integrated_circuit/passive/power/metabolic_siphon/proc/test_validity(var/mob/living/complex/human/host)
...
/obj/item/integrated_circuit/passive/power/metabolic_siphon/make_energy()
	var/mob/living/complex/human/host = null
...
/obj/item/integrated_circuit/passive/power/metabolic_siphon/synthetic/test_validity(var/mob/living/complex/human/host)
	if(!host || !host.isSynthetic() || host.stat == DEAD || host.nutrition <= 10)
		...
		for(var/I in fuel)
			if(DYNAMIC_CELL_UNITS_TO_W(assembly.battery.maxcharge - assembly.battery.charge, 1) > fuel[I])
				...
				if(I == "blood")
					...
					if(data && istype(data["donor"], /mob/living/complex/human))
						var/mob/living/complex/human/H = data["donor"]
