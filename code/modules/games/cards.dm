...
/obj/item/deck/attack_hand(mob/user, list/params)
	...
	if(isstorage(loc) || src == H.r_store || src == H.l_store || loc == user) // so objects can be removed from storage containers or pockets. also added a catch-all, so if it's in the mob you'll pick it up.
		..()
