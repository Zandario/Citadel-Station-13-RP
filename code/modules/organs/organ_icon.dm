...
/obj/item/organ/external/proc/get_icon(var/skeletal)
	...
	if(force_icon)
		...
		if(!dna)
			mob_icon = new /icon('icons/mob/human_races/species/human/body.dmi', "[icon_name][gendered_icon ? "_[gender]" : ""]")
		else
			...
			if(skeletal)
				mob_icon = new /icon('icons/mob/human_races/species/skeleton/r_skeleton.dmi', "[icon_name][gender ? "_[gender]" : ""]")
			else if (robotic >= ORGAN_ROBOT && species == !SPECIES_ADHERENT)
				mob_icon = new /icon('icons/mob/human_races/cyberlimbs/robotic.dmi', "[icon_name][s_base ? "[s_base]" : ""][gender ? "_[gender]" : ""]")
			else
