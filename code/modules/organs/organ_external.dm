...
/obj/item/organ/external/take_damage(brute, burn, sharp, edge, used_weapon = null, list/forbidden_limbs = list(), permutation = 0)
	...
	if(is_damageable(brute + burn) || !CONFIG_GET(flag/limbs_can_break))
		...
		var/can_inflict = max_damage * GET_CONFIG(number/organ_health_multiplier) - (brute_dam + burn_dam)
		var/spillover = 0
		if(can_inflict)
			if (brute > 0)
			...
			can_inflict = max_damage * GET_CONFIG(number/organ_health_multiplier) - (brute_dam + burn_dam) //Refresh the can_inflict var, so burn doesn't overload the limb if it is set to take both.
	...
	if(owner && loc == owner && !is_stump())
		if(!cannot_amputate && CONFIG_GET(flag/limbs_can_break) && (brute_dam + burn_dam) >= (max_damage * GET_CONFIG(number/organ_health_multiplier)))
...
/obj/item/organ/external/proc/update_damages()
	...
	//Bone fractures
	if(CONFIG_GET(flag/bones_can_break) && brute_dam > min_broken_damage * GET_CONFIG(number/organ_health_multiplier) && !(robotic >= ORGAN_ROBOT))
		src.fracture()
...
/obj/item/organ/external/proc/mend_fracture()
	...
	if(brute_dam > min_broken_damage * GET_CONFIG(number/organ_health_multiplier))
