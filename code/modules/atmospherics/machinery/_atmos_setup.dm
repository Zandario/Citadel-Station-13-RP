...
/datum/pipe_icon_manager/proc/gen_pipe_icons()
	...
	var/icon/pipe = new('icons/obj/atmospherics/pipes.dmi')
	...
	for(var/state in pipe.IconStates())
		...
		var/image/I = image('icons/obj/atmospherics/pipes.dmi', icon_state = state)
		pipe_icons[cache_name] = I

		for(var/pipe_color in pipe_colors)
			I = image('icons/obj/atmospherics/pipes.dmi', icon_state = state)
			I.color = pipe_colors[pipe_color]
			pipe_icons[state + "[pipe_colors[pipe_color]]"] = I

	pipe = new ('icons/obj/atmospherics/heat.dmi')
	for(var/state in pipe.IconStates())
		if(!state || findtext(state, "map"))
			continue
		pipe_icons["hepipe" + state] = image('icons/obj/atmospherics/heat.dmi', icon_state = state)

	pipe = new ('icons/obj/atmospherics/junction.dmi')
	for(var/state in pipe.IconStates())
		if(!state || findtext(state, "map"))
			continue
		pipe_icons["hejunction" + state] = image('icons/obj/atmospherics/junction.dmi', icon_state = state)
...
/datum/pipe_icon_manager/proc/gen_manifold_icons()
	...
	var/icon/pipe = new('icons/obj/atmospherics/manifold.dmi')

	for(var/state in pipe.IconStates())
		if(findtext(state, "clamps"))
			var/image/I = image('icons/obj/atmospherics/manifold.dmi', icon_state = state)
		...
		if(findtext(state, "core") || findtext(state, "4way"))
			...
			manifold_icons[state] = I
			for(var/pipe_color in pipe_colors)
				I = image('icons/obj/atmospherics/manifold.dmi', icon_state = state)
				I.color = pipe_colors[pipe_color]
...
/datum/pipe_icon_manager/proc/gen_device_icons()
	...
	device = new('icons/obj/atmospherics/vent_pump.dmi')
	for(var/state in device.IconStates())
		if(!state || findtext(state, "map"))
			continue
		device_icons["vent" + state] = image('icons/obj/atmospherics/vent_pump.dmi', icon_state = state)

	device = new('icons/obj/atmospherics/vent_scrubber.dmi')
	for(var/state in device.IconStates())
		if(!state || findtext(state, "map"))
			continue
		device_icons["scrubber" + state] = image('icons/obj/atmospherics/vent_scrubber.dmi', icon_state = state)

/datum/pipe_icon_manager/proc/gen_omni_icons()
	if(!omni_icons)
		omni_icons = new()

	var/icon/omni = new('icons/obj/atmospherics/omni_devices_vr.dmi')

	for(var/state in omni.IconStates())
		if(!state || findtext(state, "map"))
			continue
		omni_icons[state] = image('icons/obj/atmospherics/omni_devices_vr.dmi', icon_state = state)
...
/datum/pipe_icon_manager/proc/gen_underlay_icons()
	...
	var/icon/pipe = new('icons/obj/atmospherics/pipe_underlays.dmi')
	...
	for(var/state in pipe.IconStates())
		...
		for(var/D in GLOB.cardinal)
			var/image/I = image('icons/obj/atmospherics/pipe_underlays.dmi', icon_state = state, dir = D)
			underlays[cache_name + "[D]"] = I
			for(var/pipe_color in pipe_colors)
				I = image('icons/obj/atmospherics/pipe_underlays.dmi', icon_state = state, dir = D)
				I.color = pipe_colors[pipe_color]
