...
/mob/living
	...
	var/absorbing_prey = FALSE			// Determines if the person is using the succubus drain or not. See station_special_abilities_vr.
	var/drain_finalized = FALSE			// Determines if the succubus drain will be KO'd/absorbed. Can be toggled on at any time.
	var/fuzzy = TRUE					// Preference toggle for sharp/fuzzy icon.
	var/tail_alt = FALSE				// Tail layer toggle.
...
/mob/living/proc/get_smell_message(allow_generic = TRUE)
