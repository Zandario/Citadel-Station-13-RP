		...
		if((H.species.name in remainless_species) || H.isSynthetic())	//Don't leave anything if there is nothing to leave
			...
			switch(H.species.name)	//oh boy here we go, finding us a right skull
				...
				if(SPECIES_TAJ)
					...
					skull_amount--
				if(SPECIES_UNATHI)
					new /obj/item/digestion_remains/skull/unathi(src,owner)
					skull_amount--
				if(SPECIES_SKRELL)
					new /obj/item/digestion_remains/skull/skrell(src,owner)
					skull_amount--
				if(SPECIES_VASILISSAN)
					new /obj/item/digestion_remains/skull/vasilissan(src,owner)
					skull_amount--
				if(SPECIES_AKULA)
					new /obj/item/digestion_remains/skull/akula(src,owner)
					skull_amount--
				if(SPECIES_RAPALA)
					new /obj/item/digestion_remains/skull/rapala(src,owner)
					skull_amount--
				if(SPECIES_VULPKANIN)
					new /obj/item/digestion_remains/skull/vulpkanin(src,owner)
					skull_amount--
				if(SPECIES_SERGAL)
				...
				if(SPECIES_ZORREN_FLAT, SPECIES_ZORREN_HIGH)
					...
					skull_amount--
				if(SPECIES_NEVREAN)
					new /obj/item/digestion_remains/skull/nevrean(src,owner)
					skull_amount--
				if(SPECIES_TESHARI)
					new /obj/item/digestion_remains/skull/teshari(src,owner)
					skull_amount--
				if(SPECIES_VOX)
					new /obj/item/digestion_remains/skull/vox(src,owner)
					skull_amount--
				if(SPECIES_XENOHYBRID)
					new /obj/item/digestion_remains/skull/xenohybrid(src,owner)
					skull_amount--
...
/obj/item/digestion_remains
	...
	icon = 'icons/obj/item/bones_vr.dmi'
	icon_state = "generic"
	force = 0
	throw_force = 0
	item_state = "bone"
