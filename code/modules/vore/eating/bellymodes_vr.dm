	...
	for(var/A in touchable_atoms)
		...
		if(isitem(A) && !did_an_item)
			...
			if((item_digest_mode == IM_DIGEST_FOOD || item_digest_mode == IM_DIGEST) && prob(25))
				...
				// More gurgles the better~		// The things I have to update for us... -ZAN
		...
		switch(digest_mode)
			if(DM_DIGEST)
				...
				if(offset) // If any different than default weight, multiply the % of offset.
					// 4.5 nutrition points per health point. Normal same size 100+100 health prey with average weight would give 900 points if the digestion was instant.
					// With all the size/weight offset taxes plus over time oxyloss+hunger taxes deducted with non-instant digestion,
					// this should be enough to not leave the pred starved.
					owner.nutrition += offset*((nutrition_percent / 100) * 4.5 * (damage_gain) / difference)
				else
