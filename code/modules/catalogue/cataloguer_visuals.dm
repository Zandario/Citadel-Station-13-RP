...
obj/item/cataloguer/proc/draw_box(atom/A, box_size, client/C)
	...
	. += draw_line(A, SOUTHWEST, (-box_size * WORLD_ICON_SIZE) + pixel_x_correction, (-box_size * WORLD_ICON_SIZE) + pixel_y_correction, C)

	// Make a line on the bottom, going right.
	for(var/i = 1 to (box_size * 2) - 1)
		var/x_displacement = (-box_size * WORLD_ICON_SIZE) + (WORLD_ICON_SIZE * i) + pixel_x_correction
		var/y_displacement = (-box_size * WORLD_ICON_SIZE) + pixel_y_correction
		. += draw_line(A, SOUTH, x_displacement, y_displacement, C)

	// Bottom-right corner.
	. += draw_line(A, SOUTHEAST, (box_size * WORLD_ICON_SIZE) + pixel_x_correction, (-box_size * WORLD_ICON_SIZE) + pixel_y_correction, C)

	// Second line, for the right side going up.
	for(var/i = 1 to (box_size * 2) - 1)
		var/x_displacement = (box_size * WORLD_ICON_SIZE) + pixel_x_correction
		var/y_displacement = (-box_size * WORLD_ICON_SIZE) + (WORLD_ICON_SIZE * i) + pixel_y_correction
		. += draw_line(A, EAST, x_displacement, y_displacement, C)

	// Top-right corner.
	. += draw_line(A, NORTHEAST, (box_size * WORLD_ICON_SIZE) + pixel_x_correction, (box_size * WORLD_ICON_SIZE) + pixel_y_correction, C)
	...
	// Top-left corner.
	. += draw_line(A, NORTHWEST, (-box_size * WORLD_ICON_SIZE) + pixel_x_correction, (box_size * WORLD_ICON_SIZE) + pixel_y_correction, C)
