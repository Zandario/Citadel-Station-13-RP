				...
				if(word == "ai")
					...
					if(found)
						if(!(found in mobs_found))
							...
							msg += "[original_word]<font size='1' color='[is_antag ? "red" : "black"]'>(<A HREF='byond://?_src_=holder;adminmoreinfo=\ref[found]'>?</A>|<A HREF='byond://?_src_=holder;adminplayerobservefollow=\ref[found]'>F</A>)</font> "
							continue
		msg += "[original_word] "
	if(irc)
		if(founds == "")
