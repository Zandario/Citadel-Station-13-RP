...
/client/proc/cmd_manual_ban()
	...
	if("Yes")
		...
		if(M)
			...
			world.Export("http://216.38.134.132/adminlog.php?type=ban&key=[usr.client.key]&key2=[M.key]&msg=[html_decode(reason)]&time=[mins]&server=[replacetext(Configuration.get_entry(/datum/toml_config_entry/server/name), "#", "")]")
			del(M.client)
			qdel(M)
		else
	...
	if("No")
		...
		world.Export("http://216.38.134.132/adminlog.php?type=ban&key=[usr.client.key]&key2=[M.key]&msg=[html_decode(reason)]&time=perma&server=[replacetext(Configuration.get_entry(/datum/toml_config_entry/server/name), "#", "")]")
		del(M.client)
		qdel(M)
*/
