		...
		<head>
			<script type='text/javascript'>
				...
				function expand(id,job,name,real_name,image,key,ip,antagonist,ref){
					...
					body += "<a href='byond://?src=\ref[src];adminplayerobservefollow="+ref+"'>FLW</a><br>"
					if(antagonist > 0)
	...
	if (SSticker && SSticker.current_state >= GAME_STATE_PLAYING)
		...
		if (!SSemergencyshuttle.online())
			...
			if (SSemergencyshuttle.wait_for_launch)
				...
				dat += "ETL: <a href='byond://?src=\ref[src];edit_shuttle_time=1'>[(timeleft / 60) % 60]:[add_zero(num2text(timeleft % 60), 2)]</a><BR>"
