...
/proc/debug_variable(name, value, level, datum/D, sanitize = TRUE, display_flags = NONE) as text //if D is a list, name will be index, and value will be assoc value.
	...
	if (isnull(value))
		...
		item = "<a href='?_src_=vars;[HrefToken()];[VV_HK_VIEW_APPEARANCE]=[ref(value)]'>[name_part] [ref(value)]</a> = <span class='value'>/appearance</span>"
		...
		if (L.len > 0 && !(name == "underlays" || name == "overlays" || D == GLOB || L.len > (IS_NORMAL_LIST(L) ? VV_NORMAL_LIST_NO_EXPAND_THRESHOLD : VV_SPECIAL_LIST_NO_EXPAND_THRESHOLD)))
			...
			item = "<a href='?_src_=vars;[HrefToken()];Vars=[REF(value)]'>[name_part] = /list ([L.len])</a><ul>[items.Join()]</ul>"
		else
			item = "<a href='?_src_=vars;[HrefToken()];Vars=[REF(value)]'>[name_part] = /list ([L.len])</a>"
