...
/datum/admins/proc/show_player_panel(var/mob/M in GLOB.mob_list)
	...
	body += {"
		...
		<A href='byond://?src=\ref[src];oocban=[M.ckey]'>[is_role_banned_ckey(M.ckey, role = BAN_ROLE_OOC)? "<font color='red'>OOC Ban</font>" : "OOC Ban"]</A> |
	...
	for(var/datum/language/L as anything in SScharacters.all_languages())
		if(!(L.language_flags & LANGUAGE_INNATE))
			...
			if(L in M.languages)
				body += "<a href='byond://?src=\ref[src];toglang=\ref[M];lang=[html_encode(L.name)]' style='color:#006600'>[L.name]</a>"
			else
				body += "<a href='byond://?src=\ref[src];toglang=\ref[M];lang=[html_encode(L.name)]' style='color:#ff0000'>[L.name]</a>"
	...
	switch(admincaster_screen)
		if(0)
			...
			dat+={"<HR><B>Feed Security functions:</B><BR>
				...
				<BR><A href='byond://?src=\ref[src];ac_menu_censor_channel=1'>Mark Feed Channel with [(LEGACY_MAP_DATUM).company_name] D-Notice (disables and locks the channel.</A>
		...
		if(3)
			dat+={"
				...
				<HR><B><A href='byond://?src=\ref[src];ac_set_channel_receiving=1'>Receiving Channel</A>:</B> [src.admincaster_feed_channel.channel_name]<BR>
...
/datum/admins/proc/Game()
	...
	dat += {"
		...
		<br><A href='byond://?src=\ref[src];atmos_vsc=1'>Modify Atmospherics Properties</A><br>
		"}
...
/proc/get_options_bar(whom, detail = 2, name = 0, link = 1, highlight_special = 1)
	...
	switch(detail)
		...
		if(2)	//Admins
			var/ref_mob = "\ref[M]"
			return "<b>[key_name(C, link, name, highlight_special)](<A HREF='byond://?_src_=holder;adminmoreinfo=[ref_mob]'>?</A>) (<A HREF='byond://?_src_=holder;adminplayeropts=[ref_mob]'>PP</A>) (<A HREF='byond://?_src_=vars;Vars=[ref_mob]'>VV</A>) (<A HREF='byond://?_src_=holder;subtlemessage=[ref_mob]'>SM</A>) ([admin_jump_link(M)]) (<A HREF='byond://?_src_=holder;check_antagonist=1'>CA</A>) (<A HREF='byond://?_src_=holder;take_question=\ref[M]'>TAKE</A>)</b>"

		if(3)	//Devs
			var/ref_mob = "\ref[M]"
			return "<b>[key_name(C, link, name, highlight_special)](<A HREF='byond://?_src_=vars;Vars=[ref_mob]'>VV</A>)([admin_jump_link(M)]) (<A HREF='byond://?_src_=holder;take_question=\ref[M]'>TAKE</A>)</b>"

		if(4)	//Event Managers
			var/ref_mob = "\ref[M]"
			return "<b>[key_name(C, link, name, highlight_special)] (<A HREF='byond://?_src_=holder;adminmoreinfo=\ref[M]'>?</A>) (<A HREF='byond://?_src_=holder;adminplayeropts=[ref_mob]'>PP</A>) (<A HREF='byond://?_src_=vars;Vars=[ref_mob]'>VV</A>) (<A HREF='byond://?_src_=holder;subtlemessage=[ref_mob]'>SM</A>) ([admin_jump_link(M)]) (<A HREF='byond://?_src_=holder;take_question=\ref[M]'>TAKE</A>)</b>"
...
/datum/admins/proc/faxCallback(var/obj/item/paper/admin/P, var/obj/machinery/photocopier/faxmachine/destination)
	...
	if(destination.receivefax(P))
		...
		if(P.sender) // sent as a reply
			...
			for(var/client/C in GLOB.admins)
				if((R_ADMIN | R_MOD) & C.holder.rights)
					to_chat(C, "<span class='admin'><span class='prefix'>FAX LOG:</span>[key_name_admin(src.owner)] replied to a fax message from [key_name_admin(P.sender)] (<a href='byond://?_src_=holder;AdminFaxView=\ref[rcvdcopy]'>VIEW</a>)</span>")
		else
			log_admin("[key_name(src.owner)] has sent a fax message to [destination.department]")
			for(var/client/C in GLOB.admins)
				if((R_ADMIN | R_MOD) & C.holder.rights)
					to_chat(C, "<span class='admin'><span class='prefix'>FAX LOG:</span>[key_name_admin(src.owner)] has sent a fax message to [destination.department] (<a href='byond://?_src_=holder;AdminFaxView=\ref[rcvdcopy]'>VIEW</a>)</span>")
