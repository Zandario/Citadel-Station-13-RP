...
/datum/tgs_chat_command/ircstatus/Run(datum/tgs_chat_user/sender, params)
	...
	status += "Players: [LAZYLEN(GLOB.clients)]" //(Active: [get_active_player_count(0,1,0)]). Mode: [SSticker.mode ? SSticker.mode.name : "Not started"]."
	return status
...
/datum/tgs_chat_command/irccheck/Run(datum/tgs_chat_user/sender, params)
	...
	return "[LAZYLEN(GLOB.clients)] players on [GLOB.using_map.name], Mode: [master_mode]; round [current_state] -- Duration [roundduration2text()] -- [server ? server : "[world.internet_address]:[world.port]"]"
