/// This test is used to make sure a flesh-and-bone base human can suffer all the types of wounds, and that suffering more severe wounds removes and replaces the lesser wound. Also tests that [/mob/living/complex/proc/fully_heal] removes all wounds
/datum/unit_test/test_human_base/Run()
	var/mob/living/complex/human/victim = allocate(/mob/living/complex/human)
