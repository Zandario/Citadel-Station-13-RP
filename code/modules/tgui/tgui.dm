...
/tgui {
	 ...
	 * optional parent_ui tgui The parent of this UI.
	 *
	 * return tgui The requested UI.
	 */
	New(
		var/mob/user,
		var/datum/src_object,
		...
		log_tgui(
			user = user,
			message = "new [interface] fancy [user?.client?.prefs.tgui_fancy]",
	...
	proc/send_full_update(
		...
		if(!COOLDOWN_FINISHED(src, refresh_cooldown)) {
			refreshing = max(
				refreshing,
				hard_refresh ? UI_HARD_REFRESHING : UI_SOFT_REFRESHING,
			)
	...
	proc/send_update(
		...
		window.send_message(
			type = "update",
			payload = get_payload(with_data = should_update_data),
		)
	}
	...
	process(
		...
		if(!initialized && world.time - opened_at > TGUI_PING_TIMEOUT) {
			log_tgui(
				user = user,
				message = "Error: Zombie window detected, closing.",
				window = window,
				src_object = src_object,
			)
			close(can_be_suspended = FALSE)
		...
		if(type) {
			...
			switch(copytext(type, 1, 5)) {
				...
				if("mod/") { // module act
					...
					log_tgui(
						user = user,
						message = "Module: [action] [href_list["payload"]]",
						window = window,
						src_object = src_object,
					)
					process_status()
