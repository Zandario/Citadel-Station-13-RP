 ...
 * Renamed from tgui_modal to ui_modal because WHY is it called a conflicting name with /tg/station tgui_alert tgui_modals :)
 */
...
datum/proc/ui_modal_clear(datum/source = src)
	...
	var/datum/ui_modal/previous = GLOB.ui_modals[REF(source)]
	if(!previous)
		return FALSE
...
datum/proc/ui_modal_new(datum/source = src, datum/ui_modal/modal = null, replace_previous = TRUE, instant_update = TRUE)
	...
	var/datum/ui_modal/previous = LAZYACCESS(GLOB.ui_modals, REF(source))
	...
	LAZYSET(GLOB.ui_modals, REF(source), modal)
	if(instant_update)
		SStgui.update_uis(source)
	return TRUE
