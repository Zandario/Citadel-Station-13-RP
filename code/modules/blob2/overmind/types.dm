...
/datum/blob_type
	...
	var/spore_type = /mob/living/simple/blob/spore
...
// Spore things
/datum/blob_type/proc/on_spore_death(mob/living/simple/blob/spore/S)
	return
...
/datum/blob_type/fabrication_swarm
	...
	spore_type = /mob/living/simple/mechanical/hivebot/swarm
...
/datum/blob_type/fungal_bloom
	...
	can_build_factories = TRUE
	spore_type = /mob/living/simple/blob/spore/infesting

/datum/blob_type/fungal_bloom/on_spore_death(mob/living/simple/blob/spore/S)
	if(S.is_infesting)
...
/datum/blob_type/fulminant_organism
	...
	can_build_factories = TRUE
	spore_type = /mob/living/simple/blob/spore/weak
...
/datum/blob_type/fulminant_organism/on_expand(var/obj/structure/blob/B, var/obj/structure/blob/new_B, var/turf/T, var/mob/observer/blob/O)
	if(prob(10)) // 10% chance to make a weak spore when expanding.
		var/mob/living/simple/blob/spore/S = new spore_type(T)
		if(istype(S))
			S.overmind = O
			O.blob_mobs.Add(S)
		else
...
/datum/blob_type/fulminant_organism/on_death(obj/structure/blob/B)
	if(prob(33)) // 33% chance to make a spore when dying.
		var/mob/living/simple/blob/spore/S = new spore_type(get_turf(B))
