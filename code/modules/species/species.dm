...
/datum/species
	...
	var/damage_overlays = 'icons/mob/human_races/species/human/dam_human.dmi'
	var/damage_mask = 'icons/mob/human_races/species/human/dam_mask_human.dmi'
...
/datum/species/proc/get_description(var/header, var/append, var/verbose = TRUE, var/skip_detail, var/skip_photo)
	...
	if((!skip_photo && preview_icon) || !skip_detail)
		...
		if(!skip_detail)
			...
			dat += "</br><b>They breathe [GLOB.meta_gas_names[breath_type]].</b>"
			dat += "</br><b>They exhale [GLOB.meta_gas_names[exhale_type]].</b>"
			dat += "</br><b>[capitalize(english_list(poison_type))] [LAZYLEN(poison_type) == 1 ? "is" : "are"] poisonous to them.</b>"
			dat += "</small>"
		dat += "</td>"
	dat += "</tr>"
	dat += "</table><hr/>"
