...
/mob/proc/hear_say(var/message, var/verb = "says", var/datum/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	...
	if (language && (language.flags & LANGUAGE_FLAG_NONVERBAL))
		if (!speaker || (src.sdisabilities & BLIND || src.blinded) || !(speaker in view(src)))
			message = stars(message)

	if(!(language && (language.flags & LANGUAGE_FLAG_INNATE))) // skip understanding checks for LANGUAGE_FLAG_INNATE languages
	...
	if(is_deaf())
		if(!language || !(language.flags & LANGUAGE_FLAG_INNATE)) // LANGUAGE_FLAG_INNATE is the flag for audible-emote-language, so we don't want to show an "x talks but you cannot hear them" message if it's set
			if(speaker == src)
