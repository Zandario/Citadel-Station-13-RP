...
/mob/living/carbon/handle_breathing()
	if(air_master.current_cycle%4==2 || failed_last_breath || (health < CONFIG_GET(health_threshold_crit))) //First, resolve location and get a breath
		breathe()
...
/mob/living/carbon/proc/breathe()
	...
	if(health < CONFIG_GET(health_threshold_crit) && !(CE_STABLE in chem_effects)) //crit aka circulatory shock
		AdjustLosebreath(1)
