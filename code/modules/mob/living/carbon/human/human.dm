...
/mob/living/carbon/human/get_breath_from_environment(volume_needed = BREATH_VOLUME)
	var/datum/gas_mixture/breath = ..(volume_needed)
	var/turf/T = get_turf(src)
	if(istype(T) && T.is_flooded(lying) && should_have_organ(O_LUNGS))
...
/mob/living/carbon/human/get_footstep(footstep_type)
