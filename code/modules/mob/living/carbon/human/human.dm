...
/mob/living/carbon/human/proc/set_species(var/new_species, var/default_colour, var/regen_icons = TRUE, var/mob/living/carbon/human/example = null)	//VOREStation Edit - send an example
	...
	else if(species.base_color) // Apply colour.
	...
	if(!affecting)
		...
		switch(target_zone)
			if(BP_HEAD) // If targeting head, check helmets
				// If they're wearing a head piece, if that head piece is thick, the injector doesn't bypass thickness,
				// and that headpiece isn't a space helmet with an injection port - it fails.
				if(head && (head.item_flags & THICKMATERIAL) && !ignore_thickness && !istype(head, /obj/item/clothing/head/helmet/space))
					. = 0
			else // Otherwise, if not targeting head, check the suit
				// If they're wearing a suit piece, if that suit piece is thick, the injector doesn't bypass thickness,
				// and that suit isn't a space suit with an injection port - it fails.
				if(wear_suit && (wear_suit.item_flags & THICKMATERIAL) && !ignore_thickness&& !istype(wear_suit, /obj/item/clothing/suit/space))
					. = 0
	if(!. && error_msg && user)
		if(!fail_msg)
