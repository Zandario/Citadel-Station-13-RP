...
/mob/living/simple/illusion/update_icon() // We don't want the appearance changing AT ALL unless by copy_appearance().
	return

/mob/living/simple/illusion/proc/copy_appearance(atom/movable/thing_to_copy)
	if(!thing_to_copy)
		return FALSE
...
/mob/living/simple/illusion/examine(mob/user)
	if(copying)
		copying.examine(user)
		return


/mob/living/simple/illusion/bullet_act(obj/item/projectile/P)
	if(!P)
		return
...
/mob/living/simple/illusion/attack_hand(mob/living/complex/human/M)
	if(!realistic)
...
/mob/living/simple/illusion/hit_with_weapon(obj/item/I, mob/living/user, effective_force, hit_zone)
	if(realistic)
		return ..()
