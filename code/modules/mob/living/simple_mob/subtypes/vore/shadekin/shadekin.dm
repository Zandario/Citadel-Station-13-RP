/mob/living/simple/shadekin //Spawning the prototype spawns a random one, see initialize()
	name = "shadekin"
...
/mob/living/simple/shadekin/Initialize(mapload)
	//You spawned the prototype, and want a totally random one.
	if(type == /mob/living/simple/shadekin)

		//I'm told by VerySoft these are the liklihood values
		var/list/sk_types = list(
			/mob/living/simple/shadekin/red = 20,	//Actively seek people out to nom, so fairly common to see (relatively speaking),
			/mob/living/simple/shadekin/blue = 15,	//Explorers that like to interact with people, so still fairly common,
			/mob/living/simple/shadekin/purple = 15,	//Also explorers that may or may not homf people,
			/mob/living/simple/shadekin/yellow = 1	//Very rare, usually never leaves their home
		)
...
/mob/living/simple/shadekin/Destroy()
	QDEL_LIST_NULL(shadekin_abilities)
	. = ..()

/mob/living/simple/shadekin/init_vore()
...
/mob/living/simple/shadekin/Life(seconds, times_fired)
	if((. = ..()))
		return
...
/mob/living/simple/shadekin/BiologicalLife(seconds, times_fired)
	if((. = ..()))
		return
...
/mob/living/simple/shadekin/update_icon()
	. = ..()
...
/mob/living/simple/shadekin/statpanel_data(client/C)
	. = ..()
...
/mob/living/simple/shadekin/death(gibbed, deathmessage = "phases to somewhere far away!")
	cut_overlays()
	icon_state = ""
	flick("tp_out",src)
	spawn(1 SECOND)
...
/mob/living/simple/shadekin/is_incorporeal()
	if(ability_flags & AB_PHASE_SHIFTED)
		return TRUE
	return FALSE

/mob/living/simple/shadekin/handle_atmos()
	if(ability_flags & AB_PHASE_SHIFTED)
		return
	else
		return .=..()

/mob/living/simple/shadekin/proc/handle_shade()
...
/mob/living/simple/shadekin/speech_bubble_appearance()
	return "ghost"

/mob/living/simple/shadekin/apply_melee_effects(var/atom/A)
	. = ..(A)
...
/mob/living/simple/shadekin/extra_huds(var/datum/hud/hud,var/icon/ui_style,var/list/hud_elements)
	//Darkness hud
...
/mob/living/simple/shadekin/attack_hand(mob/living/complex/human/M as mob)
	. = ..()
