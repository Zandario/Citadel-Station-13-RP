...
/mob/living/complex/proc/release_control()
	...
	if(B && B.host_brain)
		...
		remove_verb(src, /mob/living/complex/proc/punish_host)
		remove_verb(src, /mob/living/complex/proc/spawn_larvae)

	else
		to_chat(src, "<span class='danger'>ERROR NO BORER OR BRAINMOB DETECTED IN THIS MOB, THIS IS A BUG !</span>")

//Brain slug proc for tormenting the host.
/mob/living/complex/proc/punish_host()
	set category = "Abilities"
...
/mob/living/complex/proc/spawn_larvae()
	set category = "Abilities"
	set name = "Reproduce"
	...
	if(B.chemicals >= 100)
		...
		vomit(1)
		new /mob/living/simple/animal/borer(get_turf(src))
