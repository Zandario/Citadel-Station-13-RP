/mob/living/complex/Initialize(mapload)
	. = ..()
...
/mob/living/complex/BiologicalLife(seconds, times_fired)
	if((. = ..()))
		return
...
/mob/living/complex/rejuvenate()
	bloodstr.clear_reagents()
	ingested.clear_reagents()
	touching.clear_reagents()
	..()

/mob/living/complex/gib()
...
/mob/living/complex/attack_hand(mob/M as mob)
	if(!istype(M, /mob/living/complex)) return ..()
	if (ishuman(M))
...
/mob/living/complex/proc/help_shake_act(mob/living/complex/M)
	if(src.health >= config_legacy.health_threshold_crit)
		if(src == M && istype(src, /mob/living/complex/human))
			...
			if (istype(src,/mob/living/complex/human) && src:w_uniform)
				var/mob/living/complex/human/H = src
			...
			if(show_ssd && !client && !teleop && (!istype(H) || !H.override_ssd))
				...
				var/mob/living/complex/human/hugger = M
...
/mob/living/complex/proc/eyecheck()
	return 0

/mob/living/complex/flash_eyes(intensity = FLASH_PROTECTION_MODERATE, override_blindness_check = FALSE, affect_silicon = FALSE, visual = FALSE, type = /atom/movable/screen/fullscreen/tiled/flash)
...
/mob/living/complex/proc/getDNA()
	return dna

/mob/living/complex/proc/setDNA(var/datum/dna/newDNA)
	dna = newDNA

// ++++ROCKDTBEN++++ MOB PROCS //END

/mob/living/complex/fire_act(datum/gas_mixture/air, exposed_temperature, exposed_volume)
	..()
	var/temp_inc = max(min(BODYTEMP_HEATING_MAX*(1-get_heat_protection()), exposed_temperature - bodytemperature), 0)
	bodytemperature += temp_inc

/mob/living/complex/can_use_hands()
	if(handcuffed)
		return 0
...
/mob/living/complex/restrained()
	if (handcuffed)
		return 1
	return

//generates realistic-ish pulse output based on preset levels
/mob/living/complex/proc/get_pulse(var/method)	//method 0 is for hands, 1 is for machines, more accurate
...
/mob/living/complex/verb/mob_sleep()
	set name = "Sleep"
	set category = "IC"
...
/mob/living/complex/Bump(atom/A)
	. = ..()
	if(istype(A, /mob/living/complex) && prob(10))
		spread_disease_to(A, "Contact")

/mob/living/complex/cannot_use_vents()
	return

/mob/living/complex/slip(var/slipped_on,stun_duration=8)
	if(buckled)
		return 0
	stop_pulling()
...
/mob/living/complex/proc/add_chemical_effect(var/effect, var/magnitude = 1)
	if(effect in chem_effects)
		chem_effects[effect] += magnitude
	else
		chem_effects[effect] = magnitude

/mob/living/complex/get_default_language()
	if(default_language)
...
/mob/living/complex/proc/should_have_organ(var/organ_check)
	return 0

/mob/living/complex/can_feel_pain(var/check_organ)
	if(isSynthetic())
		return 0
	return !(species.species_flags & NO_PAIN)

/mob/living/complex/needs_to_breathe()
	if(does_not_breathe)
		return FALSE
	return ..()

/mob/living/complex/proc/set_nutrition(amount)
	nutrition = clamp(amount, 0, initial(nutrition) * 1.5)

/mob/living/complex/proc/adjust_nutrition(amount)
	set_nutrition(nutrition + amount)

/mob/living/complex/proc/set_hydration(amount)
...
/mob/living/complex/proc/adjust_hydration(amount)
	set_hydration(hydration + amount)

/mob/living/complex/proc/update_handcuffed()
	if(handcuffed)
		drop_all_held_items()
		stop_pulling()
...
/mob/living/complex/check_obscured_slots()
	// if(slot)
