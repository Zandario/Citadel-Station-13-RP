/mob/living/complex/slime/BiologicalLife(seconds, times_fired)
	if((. = ..()))
		return
...
/mob/living/complex/slime/handle_environment(datum/gas_mixture/environment)
	if(!environment)
...
/mob/living/complex/slime/proc/adjust_body_temperature(current, loc_temp, boost)
...
/mob/living/complex/slime/handle_chemicals_in_body()
	chem_effects.Cut()
...
/mob/living/complex/slime/handle_regular_UI_updates()
...
/mob/living/complex/slime/proc/handle_nutrition()
...
/mob/living/complex/slime/proc/handle_targets()
	...
	if(!Target)
		if(will_hunt(hungry) || attacked || rabid) // Only add to the list if we need to
			...
			for(var/mob/living/L in view(7,src))
				...
				if(istype(L, /mob/living/complex/human) && dna) //Ignore slime(wo)men
				...
				if(!L.canmove) // Only one slime can latch on at a time.
					var/notarget = 0
					for(var/mob/living/complex/slime/M in view(1,L))
						if(M.Victim == L)
							notarget = 1
					if(notarget)
						continue
			...
			if(targets.len > 0)
				if(attacked || rabid || hungry == 2)
					...
					for(var/mob/living/complex/C in targets)
...
/mob/living/complex/slime/proc/handle_AI()  // the master AI process
	...
	if(Target) // We're chasing the target
		...
		for(var/mob/living/complex/slime/M in view(1, Target))
			if(M.Victim == Target)
				Target = null
				AIproc = 0
				return
		...
		var/mob/living/complex/slime/frenemy
		for (var/mob/living/complex/slime/S in view(1, src))
			if (S != src)
				frenemy = S
		if (frenemy && prob(1))
...
/mob/living/complex/slime/proc/handle_speech_and_mood()
	//Mood starts here
	var/newmood = ""
	a_intent = INTENT_HELP
	if (rabid || attacked)
	...
	if (to_say)
		...
		for (var/mob/living/complex/M in view(7,src))
			if (isslime(M))
				++slimes_near
				if (M.stat == DEAD)
					++dead_slimes
...
/mob/living/complex/slime/proc/get_max_nutrition() // Can't go above it
	if (is_adult) return 1200
	else return 1000

/mob/living/complex/slime/proc/get_grow_nutrition() // Above it we grow, below it we can eat
	if (is_adult) return 1000
	else return 800

/mob/living/complex/slime/proc/get_hunger_nutrition() // Below it we will always eat
	if (is_adult) return 600
	else return 500

/mob/living/complex/slime/proc/get_starve_nutrition() // Below it we will eat before everything else
	if (is_adult) return 300
	else return 200

/mob/living/complex/slime/proc/will_hunt(var/hunger) // Check for being stopped from feeding and chasing
...
/mob/living/complex/slime/slip() //Can't slip something without legs.
	return 0
