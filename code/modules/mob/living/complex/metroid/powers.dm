/mob/living/complex/slime/proc/Wrap(var/mob/living/M) // This is a proc for the clicks
	if (Victim == M || src == M)
		Feedstop()
		return
...
/mob/living/complex/slime/proc/invalidFeedTarget(var/mob/living/M)
	if (!M || !istype(M))
		return "This subject is incomparable..."
	if (istype(M, /mob/living/complex/slime)) // No cannibalism... yet
		return "I cannot feed on other slimes..."
	if (!Adjacent(M))
		return "This subject is too far away..."
	if (istype(M, /mob/living/complex) && M.getCloneLoss() >= M.getMaxHealth() * 1.5 || istype(M, /mob/living/simple) && M.stat == DEAD)
		return "This subject does not have an edible life energy..."
	for(var/mob/living/complex/slime/met in view())
		if(met.Victim == M && met != src)
			return "The [met.name] is already feeding on this subject..."
	return 0

/mob/living/complex/slime/proc/Feedon(var/mob/living/M)
	Victim = M
	loc = M.loc
	canmove = 0
	anchored = 1
	...
	while(Victim && !invalidFeedTarget(M) && stat != 2)
		...
		if(Adjacent(M))
			...
			else if(istype(M, /mob/living/simple))
			...
			if(prob(15) && M.client && istype(M, /mob/living/complex))
				...
				else if (istype(M, /mob/living/complex))
...
/mob/living/complex/slime/proc/Feedstop()
	if(Victim)
		if(Victim.client) to_chat(Victim, "[src] has let go of your head!")
		Victim = null

/mob/living/complex/slime/proc/UpdateFeed(var/mob/M)
	if(Victim)
		if(Victim == M)
			loc = M.loc // simple "attach to head" effect!

/mob/living/complex/slime/verb/Evolve()
	set category = "Slime"
...
/mob/living/complex/slime/verb/Reproduce()
	set category = "Slime"
	...
	if(is_adult)
		if(amount_grown >= 10)
			...
			for(var/i = 1, i <= 4, i++)
				...
				var/mob/living/complex/slime/M = new /mob/living/complex/slime/(loc, t)
			...
			var/mob/living/complex/slime/new_slime = pick(babies)
