...
/mob/living/complex/human/updatehealth()
...
/mob/living/complex/human/afflict_radiation(amt, run_armor, damage_zone)
	if(species)
		amt = amt * species.radiation_mod
	return ..()

/mob/living/complex/human/adjustBrainLoss(var/amount)
...
/mob/living/complex/human/setBrainLoss(var/amount)
...
/mob/living/complex/human/getBrainLoss()
...
/mob/living/complex/human/getBruteLoss()
	var/amount = 0
...
/mob/living/complex/human/getShockBruteLoss()
	var/amount = 0
	for(var/obj/item/organ/external/O in organs)
...
/mob/living/complex/human/getActualBruteLoss()
	var/amount = 0
	for(var/obj/item/organ/external/O in organs) // Unlike the above, robolimbs DO count.
		amount += O.brute_dam
	return amount

/mob/living/complex/human/getFireLoss()
	var/amount = 0
	for(var/obj/item/organ/external/O in organs)
		if(O.robotic >= ORGAN_ROBOT && !O.vital)
			continue //*non-vital*robot limbs don't count towards death, or show up when scanned
...
/mob/living/complex/human/getShockFireLoss()
	var/amount = 0
	for(var/obj/item/organ/external/O in organs)
		if(O.robotic >= ORGAN_ROBOT)
			continue //robot limbs don't count towards shock and crit
...
/mob/living/complex/human/getActualFireLoss()
	var/amount = 0
	for(var/obj/item/organ/external/O in organs) // Unlike the above, robolimbs DO count.
		amount += O.burn_dam
	return amount
...
/mob/living/complex/human/adjustBruteLoss(var/amount,var/include_robo)
...
//'include_robo' only applies to healing, for legacy purposes, as all damage typically hurts both types of organs
/mob/living/complex/human/adjustFireLoss(var/amount,var/include_robo)
...
/mob/living/complex/human/proc/adjustBruteLossByPart(var/amount, var/organ_name, var/obj/damage_source = null)
	amount = amount*species.brute_mod
...
/mob/living/complex/human/proc/adjustFireLossByPart(var/amount, var/organ_name, var/obj/damage_source = null)
	amount = amount*species.burn_mod
	if (organ_name in organs_by_name)
		var/obj/item/organ/external/O = get_organ(organ_name)
...
/mob/living/complex/human/Stun(amount)
	if(MUTATION_HULK in mutations)
		return
	..()

/mob/living/complex/human/Weaken(amount)
	if(MUTATION_HULK in mutations)
		return
	..()

/mob/living/complex/human/Unconscious(amount)
	if(MUTATION_HULK in mutations)
		return
...
/mob/living/complex/human/proc/Stasis(amount)
	if((species.species_flags & NO_SCAN) || isSynthetic())
		in_stasis = 0
	else
		in_stasis = amount

/mob/living/complex/human/proc/getStasis()
	if((species.species_flags & NO_SCAN) || isSynthetic())
		return 0
...
/mob/living/complex/human/proc/inStasisNow()
...
/mob/living/complex/human/getCloneLoss()
	if((species.species_flags & NO_SCAN) || isSynthetic())
		cloneloss = 0
	return ..()

/mob/living/complex/human/setCloneLoss(var/amount)
	if((species.species_flags & NO_SCAN) || isSynthetic())
		cloneloss = 0
	else
		..()

/mob/living/complex/human/adjustCloneLoss(var/amount)
	..()
...
/mob/living/complex/human/getOxyLoss()
	if(!should_have_organ(O_LUNGS))
		oxyloss = 0
	return ..()

/mob/living/complex/human/adjustOxyLoss(var/amount)
	if(!should_have_organ(O_LUNGS))
		oxyloss = 0
	else
...
/mob/living/complex/human/setOxyLoss(var/amount)
	if(!should_have_organ(O_LUNGS))
		oxyloss = 0
	else
		..()

/mob/living/complex/human/getToxLoss()
	if(species.species_flags & NO_POISON)
		toxloss = 0
	return ..()

/mob/living/complex/human/adjustToxLoss(var/amount)
	if(species.species_flags & NO_POISON)
		toxloss = 0
	else
...
/mob/living/complex/human/setToxLoss(var/amount)
	if(species.species_flags & NO_POISON)
		toxloss = 0
	else
		..()
...
/mob/living/complex/human/proc/get_damaged_organs(var/brute, var/burn)
...
/mob/living/complex/human/proc/get_damageable_organs()
	var/list/obj/item/organ/external/parts = list()
	for(var/obj/item/organ/external/O in organs)
		if(O.is_damageable())
			parts += O
...
/mob/living/complex/human/heal_organ_damage(var/brute, var/burn)
...
//It automatically updates damage overlays if necesary
//It automatically updates health status
/mob/living/complex/human/take_organ_damage(var/brute = 0, var/burn = 0, var/sharp = 0, var/edge = 0, var/emp = 0)
...
//'include_robo' only applies to healing, for legacy purposes, as all damage typically hurts both types of organs
/mob/living/complex/human/heal_overall_damage(var/brute, var/burn, var/include_robo)
	var/list/obj/item/organ/external/parts = get_damaged_organs(brute,burn)
...
/mob/living/complex/human/take_overall_damage(var/brute, var/burn, var/sharp = 0, var/edge = 0, var/used_weapon = null)
...
*/
/mob/living/complex/human/proc/restore_blood()
...
*/
/mob/living/complex/human/restore_all_organs(var/ignore_prosthetic_prefs)
	for(var/obj/item/organ/external/current_organ in organs)
		current_organ.rejuvenate_legacy(ignore_prosthetic_prefs)

/mob/living/complex/human/proc/HealDamage(zone, brute, burn)
...
/mob/living/complex/human/apply_damage(var/damage = 0, var/damagetype = BRUTE, var/def_zone = null, var/blocked = 0, var/soaked = 0, var/sharp = 0, var/edge = 0, var/obj/used_weapon = null)
	if(GLOB.Debug2)
