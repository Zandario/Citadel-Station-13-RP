/mob/living/complex/human/_slot_by_item(obj/item/I)
	if(wear_suit == I)
		return SLOT_ID_SUIT
	else if(w_uniform == I)
...
/mob/living/complex/human/_item_by_slot(slot)
	switch(slot)
		if(SLOT_ID_SUIT)
			return wear_suit
		if(SLOT_ID_UNIFORM)
...
/mob/living/complex/human/_set_inv_slot(slot, obj/item/I, flags)
	switch(slot)
		if(SLOT_ID_SUIT)
			wear_suit = I
...
/mob/living/complex/human/_get_all_slots(include_restraints)
	. = ..()
	if(wear_suit)
...
/mob/living/complex/human/_get_inventory_slot_ids()
	return ..() + list(
		SLOT_ID_SUIT,
		SLOT_ID_UNIFORM,
		SLOT_ID_SHOES,
...
/mob/living/complex/human/put_in_left_hand(obj/item/I, force)
	if(!has_organ(BP_L_HAND))
		return FALSE
	return ..()

/mob/living/complex/human/put_in_right_hand(obj/item/I, force)
	if(!has_organ(BP_R_HAND))
		return FALSE
	return ..()

// todo: this should eventually be on the datum itself probably
/mob/living/complex/human/inventory_slot_reachability_conflict(obj/item/I, slot, mob/user)
	. = ..()
	if(.)
		return
...
/mob/living/complex/human/inventory_slot_bodypart_check(obj/item/I, slot, mob/user, flags)
	. = ..()
	if(!.)
		return
...
/mob/living/complex/human/_semantic_slot_id_check(id)
	. = ..()
	if(!.)
		return
...
/mob/living/complex/human/verb/quick_equip()
...
/mob/living/complex/human/proc/smart_equipbag() // take most recent item out of bag or place held item in bag
	if(incapacitated())
		return
...
/mob/living/complex/human/proc/smart_equipbelt() // put held thing in belt or take most recent item out of belt
	if(incapacitated())
		return
	var/obj/item/thing = get_active_held_item()
