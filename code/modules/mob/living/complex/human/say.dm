/mob/living/complex/human/say(var/message,var/whispering=0)
	var/alt_name = ""
	if(name != GetVoice())
...
/mob/living/complex/human/proc/forcesay(list/append)
	if(stat == CONSCIOUS)
		if(client)
...
/mob/living/complex/human/speech_bubble_appearance()
	if(isSynthetic())
...
/mob/living/complex/human/say_understands(var/mob/other,var/datum/language/speaking = null)
	...
	if (!speaking)
		if (istype(other, /mob/living/complex/alien/diona))
...
/mob/living/complex/human/GetVoice()
...
/mob/living/complex/human/proc/SetSpecialVoice(var/new_voice)
	if(new_voice)
		special_voice = new_voice
	return

/mob/living/complex/human/proc/UnsetSpecialVoice()
	special_voice = ""
	return

/mob/living/complex/human/proc/GetSpecialVoice()
...
/mob/living/complex/human/say_quote(var/message, var/datum/language/speaking = null)
	var/verb = "says"
...
/mob/living/complex/human/handle_speech_problems(var/list/message_data)
...
/mob/living/complex/human/handle_message_mode(message_mode, message, verb, speaking, used_radios, alt_name)
	switch(message_mode)
		if("intercom")
			if(!src.restrained())
...
/mob/living/complex/human/handle_speech_sound()
