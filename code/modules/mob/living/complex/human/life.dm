...
/mob/living/complex/human/Life(seconds, times_fired)
...
/mob/living/complex/human/PhysicalLife(seconds, times_fired)
	if((. = ..()))
		return

	fall() // Prevents people from floating

/mob/living/complex/human/BiologicalLife(seconds, times_fired)
	if((. = ..()))
		return
...
/mob/living/complex/human/proc/skip_some_updates()
	if(life_tick > 5 && timeofdeath && (timeofdeath < 5 || world.time - timeofdeath > 6000))	//We are long dead, or we're junk mobs spawned like the clowns on the clown shuttle
		return TRUE
	return FALSE

/mob/living/complex/human/breathe()
	if(!inStasisNow())
		..()
...
/mob/living/complex/human/proc/get_pressure_weakness(pressure)
	if(pressure == null)
...
/mob/living/complex/human/calculate_affecting_pressure(var/pressure)
...
/mob/living/complex/human/handle_disabilities()
	..()
...
/mob/living/complex/human/handle_mutations_and_radiation(seconds)
	if(inStasisNow())
		return
...
/mob/living/complex/human/proc/radiation_hairloss()
	f_style = "Shaved"
	h_style = "Bald"
	update_hair()
...
/mob/living/complex/human/handle_chemical_smoke(var/datum/gas_mixture/environment)
...
/mob/living/complex/human/handle_post_breath(datum/gas_mixture/breath)
	..()
	//spread some viruses while we are at it
	if(breath && virus2.len > 0 && prob(10))
		for(var/mob/living/complex/M in view(1,src))
			src.spread_disease_to(M)


/mob/living/complex/human/get_breath_from_internal(volume_needed=BREATH_VOLUME)
	if(internal)
...
/mob/living/complex/human/handle_breath(datum/gas_mixture/breath)
...
/mob/living/complex/human/handle_environment(datum/gas_mixture/environment)
	if(!environment)
		return
...
/*
/mob/living/complex/human/proc/adjust_body_temperature(current, loc_temp, boost)
...
/mob/living/complex/human/proc/stabilize_body_temperature()
...
/mob/living/complex/human/proc/get_heat_protection_flags(temperature) //Temperature is the temperature you're being exposed to.
	. = 0
...
/mob/living/complex/human/proc/get_cold_protection_flags(temperature)
	. = 0
	//Handle normal clothing
	for(var/obj/item/clothing/C in list(head,wear_suit,w_uniform,shoes,gloves,wear_mask))
		if(C)
...
/mob/living/complex/human/get_heat_protection(temperature) //Temperature is the temperature you're being exposed to.
	var/thermal_protection_flags = get_heat_protection_flags(temperature)
	return get_thermal_protection(thermal_protection_flags)

/mob/living/complex/human/get_cold_protection(temperature)
	if(MUTATION_COLD_RESIST in mutations)
...
/mob/living/complex/human/proc/get_thermal_protection(var/flags)
	.=0
	if(flags)
		if(flags & HEAD)
...
/mob/living/complex/human/handle_chemicals_in_body()
...
/mob/living/complex/human/handle_regular_UI_updates()
	if(skip_some_updates())
		return FALSE
...
/mob/living/complex/human/set_stat(var/new_stat)
	. = ..()
	if(. && stat)
		update_skin(1)

/mob/living/complex/human/handle_regular_hud_updates()
...
/mob/living/complex/human/handle_vision()
	if(stat == DEAD)
		if(client)
...
/mob/living/complex/human/proc/process_glasses(var/obj/item/clothing/glasses/G)
	. = FALSE
	if(G && G.active)
		if(G.darkness_view)
...
/mob/living/complex/human/proc/process_nifsoft_vision(var/datum/nifsoft/NS)
	. = FALSE
	if(NS && NS.active)
		if(NS.darkness_view)
...
/mob/living/complex/human/handle_random_events()
	if(inStasisNow())
		return
...
/mob/living/complex/human/handle_stomach()
	spawn(0)
		for(var/mob/living/M in stomach_contents)
			if(M.loc != src)
				stomach_contents.Remove(M)
				continue
			if(istype(M, /mob/living/complex) && stat != 2)
				if(M.stat == 2)
					M.death(1)
					stomach_contents.Remove(M)
					qdel(M)
...
/mob/living/complex/human/proc/handle_changeling()
...
/mob/living/complex/human/handle_shock()
	..()
...
/mob/living/complex/human/proc/handle_pulse()
	if(life_tick % 5)
...
/mob/living/complex/human/proc/handle_heartbeat()
	if(pulse == PULSE_NONE)
		return
...
/mob/living/complex/human/handle_fire()
	if(..())
		return
...
/mob/living/complex/human/rejuvenate()
	restore_blood()
	shock_stage = 0
	traumatic_shock = 0
	..()

/mob/living/complex/human/proc/weightgain()
...
/mob/living/complex/human/proc/process_weaver_silk()
...
/mob/living/complex/human/proc/handle_nif()
	if(!nif) return
