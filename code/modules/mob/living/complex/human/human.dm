 ...
 */
/mob/living/complex/human/Initialize(mapload, datum/species/specieslike)
...
/mob/living/complex/human/LateInitialize()
	. = ..()
	regenerate_icons()
	update_transform()
...
/mob/living/complex/human/prepare_data_huds()
...
/mob/living/complex/human/legacy_ex_act(severity)
	if(!blinded)
		flash_eyes()
...
/mob/living/complex/human/proc/implant_loyalty(override = FALSE) // Won't override by default.
...
/mob/living/complex/human/proc/is_loyalty_implanted()
...
/mob/living/complex/human/restrained()
	if (handcuffed)
		return 1
...
/mob/living/complex/human/var/co2overloadtime = null
/mob/living/complex/human/var/temperature_resistance = T0C+75

// called when something steps onto a human
// this handles mobs on fire - mulebot and vehicle code has been relocated to /mob/living/Crossed()
/mob/living/complex/human/Crossed(var/atom/movable/AM)
	. = ..()
...
/mob/living/complex/human/proc/get_authentification_rank(var/if_no_id = "No id", var/if_no_job = "No job")
...
/mob/living/complex/human/proc/get_assignment(var/if_no_id = "No id", var/if_no_job = "No job")
	var/obj/item/pda/pda = wear_id
	if (istype(pda))
		if (pda.id)
...
//Useful when player do something with computers
/mob/living/complex/human/proc/get_authentification_name(var/if_no_id = "Unknown")
	var/obj/item/pda/pda = wear_id
	if (istype(pda))
		if (pda.id)
...
/mob/living/complex/human/proc/get_visible_name()
...
/mob/living/complex/human/proc/get_face_name()
...
/mob/living/complex/human/proc/get_id_name(var/if_no_id = "Unknown")
	. = if_no_id
...
/mob/living/complex/human/proc/get_idcard()
	if(wear_id)
		return wear_id.GetID()

//Removed the horrible safety parameter. It was only being used by ninja code anyways.
//Now checks siemens_coefficient of the affected area by default
/mob/living/complex/human/electrocute_act(var/shock_damage, var/obj/source, var/base_siemens_coeff = 1.0, var/def_zone = null)
...
/mob/living/complex/human/Topic(href, href_list)
						...
						if (R.fields["id"] == E.fields["id"])
							if(hasHUD(usr,"security"))
								...
								if(istype(usr,/mob/living/complex/human))
									var/mob/living/complex/human/U = usr
	...
	..()
	return
/mob/living/complex/human/needs_to_breathe()
	return !!organs_by_name[O_LUNGS] || ..()

///eyecheck()
///Returns a number between -1 to 2
/mob/living/complex/human/eyecheck()
...
/mob/living/complex/human/flash_eyes(var/intensity = FLASH_PROTECTION_MODERATE, override_blindness_check = FALSE, affect_silicon = FALSE, visual = FALSE, type = /atom/movable/screen/fullscreen/tiled/flash)
	if(internal_organs_by_name[O_EYES]) // Eyes are fucked, not a 'weak point'.
...
/mob/living/complex/human/proc/get_equipment_flash_protection()
...
/mob/living/complex/human/proc/headcheck(var/target_zone, var/brain_tag = "brain")
...
/mob/living/complex/human/IsAdvancedToolUser(silent)
	if(feral)
...
/mob/living/complex/human/abiotic(full_body)
	if(full_body)
...
/mob/living/complex/human/proc/check_dna()
	dna.check_integrity(src)

/mob/living/complex/human/get_species_name(examine)
...
/mob/living/complex/human/get_true_species_name()
	return species.get_true_name()

/mob/living/complex/human/proc/play_xylophone()
	if(!src.xylophone)
...
/mob/living/complex/human/check_has_mouth()
...
/mob/living/complex/human/proc/morph()
	set name = "Morph"
	set category = "Superpower"

	if(!(MUTATION_MORPH in mutations))
		remove_verb(src, /mob/living/complex/human/proc/morph)
		return
...
/mob/living/complex/human/proc/remotesay()
	set name = "Project mind"
	set category = "Superpower"

	if(!(MUTATION_REMOTE_TALK in src.mutations))
		remove_verb(src, /mob/living/complex/human/proc/remotesay)
		return

	var/list/creatures = list()
	for(var/mob/living/complex/h in GLOB.mob_list)
		creatures += h
...
/mob/living/complex/human/proc/remoteobserve()
	...
	if(!(MUTATION_REMOTE_VIEW in src.mutations))
		remoteview_target = null
		reset_perspective()
		remove_verb(src, /mob/living/complex/human/proc/remoteobserve)
		return
...
/mob/living/complex/human/get_visible_gender()
...
/mob/living/complex/human/proc/increase_germ_level(n)
	if(gloves)
		gloves.germ_level += n
	else
		germ_level += n

/mob/living/complex/human/revive()
...
/mob/living/complex/human/proc/is_lung_ruptured()
	var/obj/item/organ/internal/lungs/L = internal_organs_by_name[O_LUNGS]
	return L && L.is_bruised()

/mob/living/complex/human/proc/rupture_lung()
	var/obj/item/organ/internal/lungs/L = internal_organs_by_name[O_LUNGS]
...
/*
/mob/living/complex/human/verb/simulate()
	set name = "sim"
	set background = 1
...
/mob/living/complex/human/add_blood(mob/living/complex/human/M as mob)
	if (!..())
		return 0
	...
	update_bloodied()
	add_verb(src, /mob/living/complex/human/proc/bloody_doodle)
	return 1 //we applied blood to the item

/mob/living/complex/human/proc/get_full_print()
	if(!dna ||!dna.uni_identity)
		return
	return md5(dna.uni_identity)

/mob/living/complex/human/clean_blood(var/washshoes)
	. = ..()
...
/mob/living/complex/human/get_visible_implants(var/class = 0)
...
/mob/living/complex/human/embedded_needs_process()
	for(var/obj/item/organ/external/organ in src.organs)
		for(var/obj/item/O in organ.implants)
...
/mob/living/complex/human/proc/handle_embedded_objects()
...
/mob/living/complex/human/proc/embed_object_pain(var/obj/item/organ/external/organ, var/obj/item/O)
...
/mob/living/complex/human/verb/check_pulse()
	set category = "Object"
 ...
 */
/mob/living/complex/human/proc/set_species(datum/species/specieslike, regen_icons = TRUE, force = FALSE, skip, mob/living/complex/human/example)
	ASSERT(specieslike)
	// resolve id
	var/resolved_id
 ...
 * 3. human
 */
/mob/living/complex/human/proc/reset_species(force, initializing)
...
/mob/living/complex/human/proc/bloody_doodle()
	set category = "IC"
	...
	if (!bloody_hands)
		remove_verb(src, /mob/living/complex/human/proc/bloody_doodle)
...
/mob/living/complex/human/emp_act(severity)
	if(isSynthetic())
		switch(severity)
			if(1)
				Confuse(10)
...
/mob/living/complex/human/can_inject(var/mob/user, var/error_msg, var/target_zone, var/ignore_thickness = FALSE)
	. = 1
...
/mob/living/complex/human/print_flavor_text(var/shrink = 1)
...
/mob/living/complex/human/getDNA()
...
/mob/living/complex/human/setDNA()
	if(species.species_flags & NO_SCAN)
		return
	if(isSynthetic())
		return
...
/mob/living/complex/human/has_brain()
...
/mob/living/complex/human/has_eyes()
...
/mob/living/complex/human/slip(var/slipped_on, stun_duration=8)
...
/mob/living/complex/human/proc/relocate()
	set category = "Object"
...
/mob/living/complex/human/Check_Shoegrip()
...
/mob/living/complex/human/can_stand_overridden()
...
/mob/living/complex/human/verb/toggle_underwear()
...
/mob/living/complex/human/verb/pull_punches()
...
/mob/living/complex/human/should_have_organ(var/organ_check)
...
/mob/living/complex/human/can_feel_pain(var/obj/item/organ/check_organ)
	if(isSynthetic())
		return 0
...
/mob/living/complex/human/equip_post_job()
...
/mob/living/complex/human/proc/update_icon_special() //For things such as teshari hiding and whatnot.
 ...
 * Else species name
 */
/mob/living/complex/human/proc/get_display_species()
...
/mob/living/complex/human/get_nametag_name(mob/user)
	return name //Could do fancy stuff here?

/mob/living/complex/human/get_nametag_desc(mob/user)
	var/msg = ""
...
/mob/living/complex/human/proc/init_world_bender_hud()
...
/mob/living/complex/human/proc/cleanup_world_bender_hud()
	var/datum/atom_hud/world_bender/animals/A = GLOB.huds[WORLD_BENDER_HUD_ANIMALS]
	A.remove_from_hud(src)

/mob/living/complex/human/get_mob_riding_slots()
	return list(back, head, wear_suit)

/mob/living/complex/human/inducer_scan(obj/item/inducer/I, list/things_to_induce = list(), inducer_flags)
	. = ..()
	if(isSynthetic())
		things_to_induce += src

/mob/living/complex/human/inducer_act(obj/item/inducer/I, amount, inducer_flags)
	. = ..()
	if(!isSynthetic())
		return
...
/mob/living/complex/human/can_wield_item(obj/item/W)
...
/mob/living/complex/human/set_nutrition(amount)
	nutrition = clamp(amount, 0, species.max_nutrition * 1.5)

/mob/living/complex/human/get_bullet_impact_effect_type(var/def_zone)
...
/mob/living/complex/human/reduce_cuff_time()
	if(istype(gloves, /obj/item/clothing/gloves/gauntlets/rig))
		return 2
	return ..()

/mob/living/complex/human/check_obscured_slots()
	. = ..()
	if(wear_suit)
...
//! Pixel Offsets
/mob/living/complex/human/get_centering_pixel_x_offset(dir, atom/aligning)
	. = ..()
	// uh oh stinky
