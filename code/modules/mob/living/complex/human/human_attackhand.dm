/mob/living/complex/human/proc/get_unarmed_attack(var/mob/living/complex/human/target, var/hit_zone)
...
/mob/living/complex/human/attack_hand(mob/living/M as mob)
	...
	switch(M.a_intent)
		if(INTENT_HELP)
			if (istype(H) && attempt_to_scoop(H))
				return 0;
			if(iscomplexmob(M) && attempt_cpr_interaction(M))
				return TRUE
...
/mob/living/complex/human/proc/afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, inrange, params)
	return

/mob/living/complex/human/attack_generic(var/mob/user, var/damage, var/attack_message, var/armor_type = "melee", var/armor_pen = 0, var/a_sharp = 0, var/a_edge = 0)
...
/mob/living/complex/human/proc/grab_joint(var/mob/living/user, var/def_zone)
	var/has_grab = 0
...
/mob/living/complex/human/proc/break_all_grabs(mob/living/complex/user)
	var/success = FALSE
	if(pulling)
...
*/
/mob/living/complex/human/proc/apply_pressure(mob/living/user, var/target_zone)
...
/mob/living/complex/human/verb/check_attacks()
...
/mob/living/complex/human/check_attacks()
	var/dat = "<b><font size = 5>Known Attacks</font></b><br/><br/>"
...
/mob/living/complex/human/proc/set_default_attack(var/datum/unarmed_attack/u_attack)
