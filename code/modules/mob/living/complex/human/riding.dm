...
/mob/living/complex/human/drag_drop_buckle_interaction(atom/A, mob/user)
	if(!ismob(A) || (A == src))
...
/mob/living/complex/human/click_unbuckle_interaction(mob/user)
	if(user != src)
...
/mob/living/complex/human/proc/carry_piggyback(mob/living/complex/other, instant = FALSE, delay_mod = 1, loc_check = TRUE)
...
/mob/living/complex/human/proc/carry_fireman(mob/living/complex/other, instant = FALSE, delay_mod = 1, loc_check = TRUE)
	if(loc_check && !Adjacent(other))
		return FALSE
	visible_message(
...
/mob/living/complex/human/buckle_lying(mob/M)
