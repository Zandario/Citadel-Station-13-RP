/mob/living/complex/human/examine(mob/user)
	var/skip_gear = 0
	var/skip_body = 0
	...
	if(istype(src, /mob/living/complex/human/dummy))
...
//! End of the bs
	...
	if(src.stat)
		...
		if(istype(user, /mob/living/complex/human) && !user.stat && Adjacent(user))
	...
	if(ssd_msg && (!should_have_organ("brain") || has_brain()) && stat != DEAD)
		if(istype(src, /mob/living/complex/human/dummy)) // mannequins arent asleep
			return
		if(!key)
...
//Helper procedure. Called by /mob/living/complex/human/examine() and /mob/living/complex/human/Topic() to determine HUD access to security and medical records.
/proc/hasHUD(mob/M as mob, hudtype)
	...
	return 0
/*
/mob/living/complex/human/proc/examine_nutrition()
	if(!show_pudge()) //Some clothing or equipment can hide this.
		return null
	var/message = FALSE
...
/proc/hasHUD_vr(mob/living/complex/human/H, hudtype)
	if(H.nif)
		switch(hudtype)
			if("security")
...
/mob/living/complex/human/proc/examine_pickup_size(mob/living/H)
	var/message
...
/mob/living/complex/human/proc/examine_step_size(mob/living/H)
	var/message
...
/mob/living/complex/human/proc/examine_nif(mob/living/complex/human/H)
	if(nif && nif.examine_msg) //If you have one set, anyway.
		return SPAN_NOTICE("[nif.examine_msg]")

/mob/living/complex/human/proc/examine_chimera(mob/living/complex/human/H)
	var/t_He 	= "It" //capitalised for use at the start of each line.
	var/t_his 	= "its"
	var/t_His 	= "Its"
