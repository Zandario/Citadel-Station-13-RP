/obj/effect/fluid
	name = ""
	icon = 'icons/effects/fluids/liquids.dmi'
	anchored = TRUE
	opacity = FALSE
	mouse_opacity = FALSE
	layer = FLY_LAYER
	...
	var/equalize_avg_depth = FALSE
	var/equalize_avg_temp  = FALSE
...
/obj/effect/fluid/legacy_ex_act(severity)
	return
...
/obj/effect/fluid/update_icon()
	...
	if(fluid_amount > FLUID_DEEP)
		...
		alpha = min(FLUID_MAX_ALPHA,max(FLUID_MIN_ALPHA,ROUND_UP(255 * (fluid_amount / FLUID_DEEP))))
...
/obj/effect/flood/legacy_ex_act()
	return
