...
/decl/cultural_info/culture/human/vatgrown/get_random_name(gender)
	...
	#define FIRST capitalize(pick(gender == FEMALE ? first_names_female : first_names_male))
	#define NAME capitalize(pick(gender == FEMALE ? first_names_female : first_names_male)) + " " + capitalize(pick(last_names))
	switch(rand(1,4))
		if(1) return NAME
