...
/client/verb/ooc(msg as text)
	...
	if(!holder)
		if(!Configuration.get_entry(/datum/toml_config_entry/game/chat/toggles/ooc))
			...
			return
		if(!Configuration.get_entry(/datum/toml_config_entry/game/chat/toggles/ooc) && (mob.stat == DEAD))
...
/client/verb/looc(msg as text)
	...
	if(!holder)
		if(!Configuration.get_entry(/datum/toml_config_entry/game/chat/toggles/looc))
			to_chat(src, "<span class='danger'>LOOC is globally muted.</span>")
			return
		if(!Configuration.get_entry(/datum/toml_config_entry/game/chat/toggles/dooc) && (mob.stat == DEAD))
			to_chat(usr, "<span class='danger'>OOC for dead mobs has been turned off.</span>")
			return
		if(prefs.muted & MUTE_OOC)
			to_chat(src, "<span class='danger'>You cannot use OOC (muted).</span>")
