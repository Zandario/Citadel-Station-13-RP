...
/datum/category_item/player_setup_item/antagonism/candidacy/load_character(savefile/S)
	READ_FILE(S["be_special"], pref.be_special)
	READ_FILE(S["be_event_role"], pref.be_event_role)


/datum/category_item/player_setup_item/antagonism/candidacy/save_character(savefile/S)
	WRITE_FILE(S["be_special"], pref.be_special)
	WRITE_FILE(S["be_event_role"], pref.be_event_role)
...
/datum/category_item/player_setup_item/antagonism/candidacy/OnTopic(href, list/href_list, mob/user)
	...
	if(href_list["event_role_help"])
		to_chat(user, SPAN_BOLDNOTICE("\
			Toggles candidacy/preferences for partaking in certain kinds of events. \
			Note that this doesn't mean you necessarily will or will not be involved in a certain way, \
			but this may help administrators decide on candidates when necessary.")
		)
		return
	...
	if(href_list["event_role_toggle"])
		// Do I care about href exploits no what are you gonna do turn your own prefs off LOL @silicons // Yes @Zandario
		pref.be_event_role ^= text2num(href_list["event_role_toggle"])
		return TOPIC_REFRESH
	return ..()
