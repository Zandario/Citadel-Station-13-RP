	...
	for(var/gear_name in gear_datums)
		...
		if(G.whitelisted)
			...
			if(whitelist_cache[spec] == null)
				whitelist_cache[spec] = is_species_whitelisted(preference_mob, name_static_species_meta(spec))
			if(!whitelist_cache[spec])
				continue

		if(G.whitelisted && !is_species_whitelisted(preference_mob, name_static_species_meta(G.whitelisted)))
			continue
