...
/datum/nano_module/law_manager/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = default_state)
	var/data[0]
	owner.lawsync()
	...
	if (!ui)
		ui = new(user, src, ui_key, "law_manager.jst", sanitize("[src] - [owner]"), 800, is_malf(user) ? 600 : 400, state = state)
