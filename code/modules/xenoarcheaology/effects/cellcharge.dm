	...
	if(user)
		if(isrobot(user))
			...
			for (var/obj/item/cell/D in R.contents)
				...
				to_chat(R, "<font color=#4F49AF>SYSTEM ALERT: Large energy boost detected!</font>")
			return 1
	...
	if(holder)
		...
		for (var/obj/machinery/power/smes/S in GLOB.smeses)
			...
			if(get_dist(T, S) > src.effectrange)
				continue
			S.charge += 25
		for (var/mob/living/silicon/robot/M in GLOB.silicon_list)
			if(T.z != M.z)
				continue
			if(get_dist(T, M) > 50)
				continue
			for (var/obj/item/cell/D in M.contents)
				...
				if(world.time - last_message > 200)
					to_chat(M, "<font color=#4F49AF>SYSTEM ALERT: Energy boost detected!</font>")
