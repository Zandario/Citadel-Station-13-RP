	...
	if(href_list["flavor_text"])
		switch(href_list["flavor_text"])
			...
			if("general")
				...
				var/msg = sanitize(input(usr,"Set the flavor text for your [href_list["flavor_text"]]. Flavor text is what someone can see IC with a glance, please do not include OOC things like personality / backstory!","Flavor Text",html_decode(pref.flavor_texts[href_list["flavor_text"]])) as message, extra = 0, max_length = 8192)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
		SetFlavorText(user)
