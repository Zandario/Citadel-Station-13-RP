	...
	for(var/turf/gen_turf in base_turfs)
		...
		for(var/turf/T in trange(field_radius, gen_turf))
			...
			for(var/turf/TN in orange(1, T))
				TA = get_area(TN)
				//if ((isspaceturf(TN) || (isopenturf(TN) && (istype(TA, /area/space) || TA.area_flags & AREA_FLAG_EXTERNAL))))
				if((isspaceturf(TN) || (isopenturf(TN) && (istype(TA, /area/space)))))
					. |= TN
					continue
