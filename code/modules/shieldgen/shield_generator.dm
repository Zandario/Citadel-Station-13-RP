...
/obj/machinery/power/shield_generator/proc/fieldtype_hull()
	...
	for(var/turf/gen_turf in base_turfs)
		...
		for(var/turf/T in trange(field_radius, gen_turf))
			...
			if(istype(T, /turf/space) || isopenturf(T))
				continue
			...
			for(var/turf/TN in orange(1, T))
				TA = get_area(TN)
				//if ((istype(TN, /turf/space) || (isopenturf(TN) && (istype(TA, /area/space) || TA.area_flags & AREA_FLAG_EXTERNAL))))
				if((istype(TN, /turf/space) || (isopenturf(TN) && (istype(TA, /area/space)))))
					. |= TN
					continue
