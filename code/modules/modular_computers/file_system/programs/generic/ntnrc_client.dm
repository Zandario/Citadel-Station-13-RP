...
/datum/computer_file/program/chatclient/Topic(href, href_list)
	...
	if(href_list["PRG_toggleadmin"])
		...
		if(netadmin_mode)
			...
			return 1
		var/mob/living/user = usr
		if(can_run(usr, 1, ACCESS_SCIENCE_EXONET))
			if(channel)
...
/datum/nano_module/program/computer_chatclient/nano_ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, datum/topic_state/state = default_nano_state)
