...
/proc/get_light_type_instance(var/bulb_type)
	. = light_type_cache[bulb_type]
	if(!.)
		. = new bulb_type
		light_type_cache[bulb_type] = .
...
/obj/machinery/light/flamp
	...
	layer = OBJ_LAYER
	desc = "A floor lamp."
	bulb_type = /obj/item/light/bulb/strong
...
/obj/machinery/light/Initialize(mapload, obj/machinery/light_construct/construct)
	...
	if(construct)
		...
		if(start_with_cell && !no_emergency)
			cell = new/obj/item/cell/emergency_light(src)
		var/obj/item/light/L = get_light_type_instance(bulb_type)
		update_from_bulb(L)
