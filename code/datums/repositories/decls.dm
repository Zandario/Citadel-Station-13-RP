...
/repository/decls/proc/get_decl_by_id(decl_id)
	. = get_decl(fetched_decl_ids[decl_id])

/repository/decls/proc/get_decl(decl/decl_type)
		...
		switch(init_result)
			if(INITIALIZE_HINT_NORMAL)
				...
				if(fetched_decls[decl_type] == decl)
					fetched_decls -= decl_type
				stack_trace("Invalid return hint to [decl_type]/Initialize(): [init_result || "NULL"]")
