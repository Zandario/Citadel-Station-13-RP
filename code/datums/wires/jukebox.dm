...
datum/wires/jukebox/on_pulse(wire)
	...
	switch(wire)
		if(WIRE_MAIN_POWER1)
			holder.visible_message("<span class='notice'>[icon2html(thing = holder, target = world)] The power light flickers.</span>")
			A.shock(usr, 90)
		if(WIRE_JUKEBOX_HACK)
			holder.visible_message("<span class='notice'>[icon2html(thing = holder, target = world)] The parental guidance light flickers.</span>")
		if(WIRE_REVERSE)
			holder.visible_message("<span class='notice'>[icon2html(thing = holder, target = world)] The data light blinks ominously.</span>")
		if(WIRE_SPEEDUP)
			holder.visible_message("<span class='notice'>[icon2html(thing = holder, target = world)] The speakers squeaks.</span>")
		if(WIRE_SPEEDDOWN)
			holder.visible_message("<span class='notice'>[icon2html(thing = holder, target = world)]] The speakers rumble.</span>")
		if(WIRE_START)
			A.StartPlaying()
		if(WIRE_STOP)
			A.StopPlaying()
