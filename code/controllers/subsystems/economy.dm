...
/datum/controller/subsystem/economy/proc/charge_head_department(amount, purpose)
	if(!GLOB.using_map || !HEAD_DEPARTMENT)	// Shouldn't happen, but just in case
		return

	var/datum/department/head_account = GLOB.using_map.get_head_department()
...
/datum/controller/subsystem/economy/proc/charge_main_department(amount, purpose)
	if(!GLOB.using_map || !MAIN_DEPARTMENT)	// Shouldn't happen, but just in case
		return

	var/datum/department/main_account = GLOB.using_map.get_main_department()
