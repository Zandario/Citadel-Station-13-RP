...
/datum/controller/subsystem/fluids/fire(resumed = 0)
	...
	while (af_index <= processing_fluids.len)
		...
		if (QDELETED(F))
			...
			if(!F.loc || F.loc != F.start_loc || !F.loc.can_fluid_pass())
				qdel(F)
				continue
			...
			for(var/spread_dir in GLOB.cardinal)
				...
				if((T.fluid_blocked_dirs & coming_from) || !T.can_fluid_pass(coming_from))
					continue
	...
	while (af_index <= processing_fluids.len)
		...
		if (QDELETED(F))
			...
			if(islist(F.equalizing_fluids) && F.equalizing_fluids.len > 1)
				F.equalize_avg_depth = FLOOR(F.equalize_avg_depth/F.equalizing_fluids.len, 1)
				F.equalize_avg_temp = FLOOR(F.equalize_avg_temp/F.equalizing_fluids.len, 1)
