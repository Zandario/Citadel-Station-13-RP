...
/datum/controller/subsystem/zcopy/proc/calculate_zstack_limits()
	...
	for (var/z in 1 to world.maxz)
		if (!SSmapping.cached_level_up[z])
...
/client/proc/analyze_openturf(turf/T)
	...
	else if (T.mz_flags & MZ_OVERRIDE)
		out += "<h3>Using synthetic rendering (OVERRIDE) — override is [T.z_appearance ? "MANUAL" : "BASETURF (resolved to [READ_BASETURF(T)])"].</h3>"
