	...
	if(config_legacy.kick_inactive)
		for(var/i in GLOB.clients)
			...
			if(C.is_afk(config_legacy.kick_inactive MINUTES) && !C.holder) // Allow admins to idle
				...
				var/adminlinks
				adminlinks = " (<A HREF='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[C.mob.x];Y=[C.mob.y];Z=[C.mob.z]'>JMP</a>|<A HREF='byond://?_src_=holder;cryoplayer=\ref[C.mob]'>CRYO</a>)"
