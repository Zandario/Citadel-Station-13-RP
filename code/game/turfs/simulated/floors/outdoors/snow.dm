...
/turf/simulated/floor/outdoors/snow/attackby(obj/item/tool, mob/user)
	if(istype(tool, /obj/item/shovel))
		to_chat(user, SPAN_NOTICE("You begin to remove \the [src] with your [tool]."))
		if(do_after(user, 4 SECONDS * tool.tool_speed))
...
/turf/simulated/floor/outdoors/snow/attack_hand(mob/user)
