...
/turf/simulated/wall/update_overlays()
	...
	if (apply_reinf_overlay())
		...
		if (construction_stage != null && construction_stage < 6)
			...
			appearance.appearance_flags = RESET_COLOR | TILE_BOUND
		...
		// Directional Reinforcements.
			...
			appearance.appearance_flags = RESET_COLOR | TILE_BOUND
			appearance.layer = WALL_REINF_LAYER
			appearance.color = reinf_material.icon_colour
			. += appearance
	...
	// 	// Reinforcements Wall Toppers
	// 	if(smoothing_junction & NORTH_JUNCTION)
	// 		var/image/appearance = image(reinf_material.icon_reinf, "wall_edge")
	// 		appearance.appearance_flags = RESET_COLOR | TILE_BOUND
	// 		// appearance.color = paint_color ? paint_color : reinf_material.icon_colour
	// 		appearance.pixel_z = 32
	// 		appearance.layer = WALL_TOPPER_LAYER
	// 		appearance.color = reinf_material.icon_colour
	// 		. += appearance

	// // Wall Insets
	// if(smoothing_junction & SOUTH_JUNCTION)
	// 	var/image/appearance = image(material.icon_reinf, "wall_inner")
	// 	appearance.appearance_flags = RESET_COLOR | TILE_BOUND
