var/list/singleton/flooring/flooring_types
...
/singleton/flooring
	...
	var/has_base_range = 0
	var/has_damage_range = 0
	var/has_burn_range = 0
	var/damage_temperature = 0
	var/apply_thermal_conductivity = 0
	var/apply_heat_capacity = 0
...
/singleton/flooring/proc/drop_product(atom/A)
	if(ispath(build_type, /obj/item/stack))
		var/obj/item/stack/Stack = build_type
		new build_type(A, Stack.TEMP_FLOOR_use_cost)
	else
		new build_type(A)
