...
/datum/game_mode/epidemic/post_setup()
	...
	var/list/crew = list()
	for(var/mob/living/complex/human/H in world) if(H.client)
		// heads should not be infected
		if(H.mind.assigned_role in command_positions) continue
		crew += H

	if(crew.len < 2)
		to_chat(world, "<span class='danger'>There aren't enough players for this mode!</span>")
		to_chat(world, "<span class='danger'>Rebooting world in 5 seconds.</span>")
	...
	for(var/i = 0, i < lethal_amount, i++)
		var/mob/living/complex/human/H = pick(crew)
	...
	var/mob/living/complex/human/patient_zero = pick(infectees)
...
/datum/game_mode/epidemic/check_win()
	var/alive = 0
	var/sick = 0
	for(var/mob/living/complex/human/H in world)
...
/datum/game_mode/epidemic/declare_completion()
	if(finished == 1)
		...
		to_chat(world, "<font size = 3><span class='danger'> The crew succumbed to the epidemic!</span></font>")
	..()
	return 1
