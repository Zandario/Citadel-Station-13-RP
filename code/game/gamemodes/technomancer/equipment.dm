...
/obj/item/scepter/attack_self(mob/living/complex/human/user)
...
/obj/item/scepter/afterattack(atom/target, mob/living/complex/human/user, proximity_flag, click_parameters)
	if(proximity_flag)
		return ..()
	var/obj/item/item_to_test = user.get_other_hand(src)
	if(istype(item_to_test, /obj/item/spell))
