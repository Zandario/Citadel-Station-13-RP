...
/mob/living/simple/technomancer_golem
	name = "G.O.L.E.M."
...
/mob/living/simple/technomancer_golem/Initialize(mapload)
	. = ..()
	core = new(src)
...
/mob/living/simple/technomancer_golem/Destroy()
	qdel(core)
	qdel(sword_image)
	qdel(spell_image)
	return ..()

/mob/living/simple/technomancer_golem/unref_spell()
	active_spell = null
	return ..()
...
/mob/living/simple/technomancer_golem/update_icon()
	cut_overlays()
...
/mob/living/simple/technomancer_golem/proc/spellcast_pre_animation()
	setClickCooldown(5)
	manual_flick(spell_pre_attack_states, spell_image, reset_to = "golem_spell_attack_3")

/mob/living/simple/technomancer_golem/proc/spellcast_post_animation()
	setClickCooldown(8)
	manual_flick(spell_post_attack_states, spell_image, reset_to = "golem_spell")

/mob/living/simple/technomancer_golem/proc/sword_pre_animation()
	setClickCooldown(6)
	manual_flick(sword_pre_attack_states, sword_image)

/mob/living/simple/technomancer_golem/proc/sword_post_animation()
	setClickCooldown(3)
	manual_flick(sword_post_attack_states, sword_image, reset_to = "golem_sword")

/mob/living/simple/technomancer_golem/DoPunch(var/atom/A)
	sword_pre_animation()
	. = ..() // This does the actual attack and will check adjacency again.
	sword_post_animation()

/mob/living/simple/technomancer_golem/isSynthetic()
	return TRUE // So Mend Synthetic will work on them.

/mob/living/simple/technomancer_golem/speech_bubble_appearance()
	return "synthetic_evil"

/mob/living/simple/technomancer_golem/place_spell_in_hand(var/path)
	if(!path || !ispath(path))
		return 0
...
/mob/living/simple/technomancer_golem/verb/test_giving_spells()
...
// Used to cast spells.
/mob/living/simple/technomancer_golem/RangedAttack(var/atom/A, var/params)
	if(active_spell)
...
/mob/living/simple/technomancer_golem/UnarmedAttack(var/atom/A, var/proximity)
	if(proximity)
		if(active_spell)
			spellcast_pre_animation()
...
/mob/living/simple/technomancer_golem/get_technomancer_core()
	return core

/mob/living/simple/technomancer_golem/proc/bind_to_mob(mob/user)
	if(!user || master)
		return
	master = user
	name = "[master]'s [initial(name)]"

/mob/living/simple/technomancer_golem/examine(mob/user)
	. = ..()
...
/mob/living/simple/technomancer_golem/Life(seconds, times_fired)
	..()
	handle_ai()

// This is where the real spaghetti begins.
/mob/living/simple/technomancer_golem/proc/handle_ai()
	if(!master)
		return
...
/mob/living/simple/technomancer_golem/proc/support_friend(var/mob/living/L)
...
/mob/living/simple/technomancer_golem/proc/targeted_blink(var/atom/target)
