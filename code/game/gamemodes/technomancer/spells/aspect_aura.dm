/datum/technomancer/spell/aspect_aura
	...
	cost = 200
	obj_path = /mob/living/complex/human/proc/technomancer_aspect_aura

/mob/living/complex/human/proc/technomancer_aspect_aura()
...
/obj/item/spell/aspect_aura/on_combine_cast(obj/item/W, var/mob/living/complex/human/user)
	if(istype(W, /obj/item/spell))
		...
		spawn(1)
			switch(spell.aspect)
				if(ASPECT_FIRE)
					user.place_spell_in_hand(/obj/item/spell/aura/fire)
				if(ASPECT_FROST)
					user.place_spell_in_hand(/obj/item/spell/aura/frost)
				if(ASPECT_BIOMED)
...
/obj/item/spell/aura/fire/process(delta_time)
	...
	for(var/mob/living/complex/human/H in nearby_mobs)
...
/obj/item/spell/aura/biomed/process(delta_time)
	...
	if(regen_tick % 5 == 0)
		...
		if(heal_allies_only)
			for(var/mob/living/complex/human/H in nearby_mobs) //Heal our apprentices
				if(H.mind && technomancers.is_antagonist(H.mind))
					mobs_to_heal |= H
			for(var/mob/living/simple/hostile/SAH in nearby_mobs) //Heal our controlled mobs
