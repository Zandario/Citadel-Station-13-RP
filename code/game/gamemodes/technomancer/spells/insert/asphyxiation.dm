	...
	spawn(1)
		if(ishuman(host))
			...
			while(pulses)
				...
				//to_chat(world, "Inflicted [round(abs(health_lost * 0.25))] damage!")
				pulses--
...
/obj/item/inserted_spell/asphyxiation/proc/predict_crit(var/pulses_remaining, var/mob/living/complex/human/victim, var/previous_damage = 0)
	...
	if(previous_damage + health_lost >= victim.getMaxHealth()) // We're probably going to hardcrit
		...
		//to_chat(world, "Predicted oxycrit.")
		return 1
