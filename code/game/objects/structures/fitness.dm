...
/obj/structure/fitness/punchingbag/attack_hand(var/mob/living/complex/human/user)
	if(!istype(user))
		..()
		return
...
/obj/structure/fitness/weightlifter/attack_hand(var/mob/living/complex/human/user)
	if(!istype(user))
		return
	if(user.loc != src.loc)
