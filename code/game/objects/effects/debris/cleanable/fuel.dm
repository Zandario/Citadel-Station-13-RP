...
/obj/effect/debris/cleanable/liquid_fuel/Initialize(mapload, amt = 1, nologs = TRUE)
	...
	for(var/obj/effect/debris/cleanable/liquid_fuel/other in newLoc)
		if(other != src)
...
/obj/effect/debris/cleanable/liquid_fuel/proc/Spread(exclude=list())
	...
	for(var/d in GLOB.cardinal)
		...
		if(target.Enter(src, origin))
			var/obj/effect/debris/cleanable/liquid_fuel/other_fuel = locate() in target
			if(other_fuel)
...
/obj/effect/debris/cleanable/liquid_fuel/flamethrower_fuel/Spread()
	...
	for(var/d in list(turn(dir,90),turn(dir,-90), dir))
		...
		if(locate(/obj/effect/debris/cleanable/liquid_fuel/flamethrower_fuel) in O)
			continue
		if(O.Enter(src, S))
			var/new_pool_amount = amount * 0.25
			if(new_pool_amount > 0.1)
				var/obj/effect/debris/cleanable/liquid_fuel/flamethrower_fuel/F = new(O, new_pool_amount, d)
