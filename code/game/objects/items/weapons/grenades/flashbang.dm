...
/obj/item/grenade/flashbang/detonate()
	...
	for(var/obj/structure/closet/L in hear(max_range, get_turf(src)))
		if(locate(/mob/living/complex/, L))
			for(var/mob/living/complex/M in L)
				bang(get_turf(src), M)

	for(var/mob/living/complex/M in hear(max_range, get_turf(src)))
		bang(get_turf(src), M)
...
/obj/item/grenade/flashbang/proc/bang(var/turf/T , var/mob/living/complex/M)					// Added a new proc called 'bang' that takes a location and a person to be banged.
