...
/datum/antagonist/proc/get_check_antag_output(var/datum/admins/caller)
	...
	for(var/datum/mind/player in current_antagonists)
		...
		dat += "<tr>"
		if(M)
			dat += "<td><a href='byond://?src=\ref[src];adminplayeropts=\ref[M]'>[M.real_name]/([player.ckey])</a>"
			if(!M.client)      dat += " <i>(logged out)</i>"
			if(M.stat == DEAD) dat += " <b><font color=red>(DEAD)</font></b>"
			dat += "</td>"
			dat += "<td>\[<A HREF='byond://?src=\ref[caller];adminplayeropts=\ref[M]'>PP</A>]\[<A href='byond://?src=\ref[caller];priv_msg=\ref[M]'>PM</A>\]\[<A href='byond://?src=\ref[caller];traitor=\ref[M]'>TP</A>\]</td>"
		else
