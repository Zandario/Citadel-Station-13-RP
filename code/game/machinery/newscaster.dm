		...
		switch(screen)
			if(0)
				...
				if(securityCaster)
					...
					dat+="<BR><A href='byond://?src=\ref[src];menu_censor_channel=1'>Mark Feed Channel with [(LEGACY_MAP_DATUM).company_name] D-Notice</A>"
			...
			if(3)
				...
				dat+="<B><A href='byond://?src=\ref[src];set_attachment=1'>Attach Photo</A>:</B>  [(photo_data ? "Photo Attached" : "No Photo")]</BR>"
			...
			if(14)
				...
				dat+="<A href='byond://?src=\ref[src];set_attachment=1'>Attach Photo</A>: [(photo_data ? "Photo Attached" : "No Photo")]</BR>"
				if(wanted_already)
