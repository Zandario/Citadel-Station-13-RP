...
/obj/machinery/computer/robotics/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("stopbot") // lock or unlock the borg
			...
			if(R.connected_ai)
				to_chat(R.connected_ai, "[!R.lockcharge ? SPAN_NOTICE("NOTICE - Cyborg lockdown lifted") : SPAN_ALERT("ALERT - Cyborg lockdown detected")]: <a href='byond://?src=[REF(R.connected_ai)];track=[html_encode(R.name)]'>[R.name]</a></span><br>")
			. = TRUE
