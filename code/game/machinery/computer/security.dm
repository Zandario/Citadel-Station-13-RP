...
/obj/machinery/computer/secure_data/ui_interact(mob/user, tgui/ui = null)
		...
		switch(screen)
			...
			if(SEC_DATA_RECORD)
				...
				if(istype(active1, /datum/data/record) && data_core.general.Find(active1))
					...
					general["has_photos"] = (active1.fields["photo-south"] || active1.fields["photo-west"] ? TRUE : FALSE)
					general["empty"] = FALSE
				else
					general["empty"] = TRUE
