...
#define RL_APPLY_LIGHT_LINE(src, lx, ly, dir, radius, brightness, height2, r, g, b) do { \
	...
	if (radius <= 1) { atten *= 0.1 }\
		...
		switch(dir){ \
			...
			if (WEST){ if (ROUND_UP(lx) - src.x > 0){ atten *= mult_atten } }\
			if (EAST){ if (round(lx) - src.x < 0){ atten *= mult_atten } }\
			if (SOUTH){ if (ROUND_UP(ly) - src.y > 0){ atten *= mult_atten } }\
		}\
...
#define SHOULD_QUEUE (( SSlighting.light_update_queue || APPROX_TICK_USE > LIGHTING_MAX_TICKUSAGE || RL_Suspended) && !queued_run && SSticker.current_state > GAME_STATE_SETTING_UP)
/datum/light
	var/x
	var/y
	var/z
...
/datum/light/proc/set_brightness(brightness, queued_run = 0)
	...
	if (src.enabled)
		if (SHOULD_QUEUE)
			SSlighting.light_update_queue += src
...
/datum/light/proc/set_color(red, green, blue, queued_run = 0)
...
/datum/light/proc/set_height(height, queued_run = 0)
...
/datum/light/proc/enable(queued_run = 0)
	if (enabled)
...
/datum/light/proc/disable(queued_run = 0)
	if (!enabled)
		dirty_flags &= ~D_ENABLE
		return
...
var/RL_Started = FALSE
var/RL_Suspended = FALSE
...
/obj/overlay/tile_effect/lighting/mul/Destroy()
	var/turf/T = src.loc
...
/obj/overlay/tile_effect/lighting/add/Destroy()
	. = ..()
	var/turf/T = src.loc
...
/turf/proc/RL_Cleanup()
	/*
	if (src.RL_MulOverlay)
		src.RL_MulOverlay.setLoc(null)
		qdel(src.RL_MulOverlay)
		src.RL_MulOverlay = null
	if (src.RL_AddOverlay)
		src.RL_AddOverlay.setLoc(null)
...
/atom/movable/proc/update_directional_lights()
	if (src.dir != old_dir && src.RL_Attached && REALTIMEOFDAY > next_light_dir_update)
		next_light_dir_update = REALTIMEOFDAY + 0.2 SECONDS
		old_dir = src.dir
...
/atom/proc/RL_SetOpacity(new_opacity)
	if(QDELETED(src))
		return
