/**
 *# bodytypes
 *? - They are lists for var storage, but should only be passed one at a time to rendering!
 *
 * These are going to guzzle init times oops well unfortunately it's either that or:
 * - string concat, but since we need lookups to not include the "," we'd have to have three defines per bodytype instead of two.
 *
 * - datum paths and cached datums,
 *   which is nice but requires /datum/bodytypes/x for every new set of bodytypes which is not something we'd like to do.
 *
 *
...
#define BODYTYPES_ALL  "all"  //! All bodytypes.
#define BODYTYPES_NONE "none" //! No bodytypes.
...
//# Bodytypes - These are the actual bodytypes.

#define BODYTYPE_DEFAULT     "default"     //! Normal human bodytype. Generally everyone else.

#define BODYTYPE_ADHERENT    "adherent"    /// Adherent bodytype. Generally giant serpent creatures.
#define BODYTYPE_AKULA       "akula"       //! Akula bodytype. Generally shark.
#define BODYTYPE_NEVREAN     "nevrean"     //! Neverean bodytype.
#define BODYTYPE_PHORONOID   "phoronoid"   //! Phoronoid bodytype.
#define BODYTYPE_PROMETHEAN  "promethean"  //! Promethean bodytype.
#define BODYTYPE_SERGAL      "sergal"      //! Sergal bodytype. Generally cheese.
#define BODYTYPE_SKRELL      "skrell"      //! Skrell bodytype. Generally weird fleshy fishpeople.
#define BODYTYPE_TAJARAN     "tajaran"     //! Tajaran bodytype. Generally cat.
#define BODYTYPE_TESHARI     "teshari"     //! Teshari bodytype. Generally tiny birds.
#define BODYTYPE_UNATHI      "unathi"      //! Unathi bodytype. Generally lizard.
#define BODYTYPE_UNATHI_DIGI "unathi-digi" //! Digitigrade Unathi bodytype.
#define BODYTYPE_VOX         "vox"         //! Vox bodytype.
#define BODYTYPE_VULPKANIN   "vulpkanin"   //! Vulpkanin bodytype. Generally foxes.
#define BODYTYPE_WEREBEAST   "werebeast"   //! Werebeast bodytype.
#define BODYTYPE_XENOHYBRID  "xenohybrid"  //! Xenomorph Hybrid bodytype.
#define BODYTYPE_ZADDAT      "zaddat"      //! zaddat bodytype.
#define BODYTYPE_ZORREN_FLAT "zorren-flat" //! Flatlander Zorren bodytype.
#define BODYTYPE_ZORREN_HIGH "zorren-high" //! Highlander Zorren bodytype.
...
/**
 * TODO: What are we going to do with these? @silicons
 *
 ...
 *? How we'll do that is another problem to be solved later.
 */
/*
...
/**
 ** Why do these exist?
 ** Because bodytypes can be added/deleted all you want but as long as these stay intact, .dmis don't need editing.
...
/**
 * proc: bodytype_to_string
 *
 * Converts a bodytype to a string.
 *
 * Arguments:
 * * bodytype - The bodytype to convert.
 *
 * Returns:
 * * string - The string representation of the bodytype.
 */
