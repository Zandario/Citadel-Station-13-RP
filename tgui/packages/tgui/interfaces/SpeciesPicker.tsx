...
export const SpeciesPicker = (props, context) => {
  ...
  const [selectedSpecies, setSelectedSpecies] = useLocalState<String | null>(
    context,
    'selectedSpecies',
    data.default
  );
  ...
  const isHidden = (s: Species) =>
    s.spawn_flags & SpeciesSpawnFlags.Secret && !hasWhitelist(s);
          ...
          <Stack.Item width="25%">
            {selectedCategory === null ? (
              ...
              <Section fill scrollable title="Species">
                {species
                  .filter(
                    (s) => s.category === selectedCategory && !isHidden(s)
                  )
                  .map((s) => (
                    <Button
                      fluid
              ...
              <Section fill title={selected.name}>
                <Box
                  dangerouslySetInnerHTML={{
                    __html: sanitizeText(selected.desc),
                  }}
                />
                <Box
                  bottom="10px"
                  ...
                  {!!isRestricted(selected) && (
                    <NoticeBox danger textAlign="center">
                      ...
                      cannot join the game with it in most normal roles.
                    </NoticeBox>
                  )}
                  {!!isWhitelisted(selected) &&
                    (hasWhitelist(selected) ? (
                      <NoticeBox success textAlign="center">
                        You have the whitelist to play this species.
                      </NoticeBox>
                    ) : hasAdminWhitelist ? (
                      <NoticeBox success textAlign="center">
                        You have administrative override for this species
                        whitelist. Please play responsibly.
                      </NoticeBox>
                    ) : (
                      <NoticeBox warning textAlign="center">
                        This is a whitelisted species. You can select it, but
                        cannot join the game with it without a whitelist.
                      </NoticeBox>
                    ))}
                  <Button
                    color="transparent"
