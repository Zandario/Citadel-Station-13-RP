...
/datum/map
	...
	/// If populated, all subtypes of this type will be instantiated and used to populate the *_levels lists.
	var/zlevel_datum_type
	...
	///Z-levels exempt from xenoarch finds and digsites spawning.
	var/list/xenoarch_exempt_levels = list()
	...
	/// List of lists of zlevels to smoosh into single icons
	...
	/// Areas meteor strikes may choose to hit.
	var/list/meteor_strike_areas
	/// Are there z-levels restricted?
	var/ai_shell_restricted = FALSE
	/// Which z-levels ARE we allowed to visit?
	var/ai_shell_allowed_levels = list()
	...
	/// Determines if this map supports automated bot patrols
	var/bot_patrolling = TRUE

	var/allowed_spawns = list("Arrivals Shuttle","Gateway", "Cryogenic Storage", "Cyborg Storage")

//! ## Persistence!
	/// Used if you end the round dead.
	var/datum/spawnpoint/spawnpoint_died = /datum/spawnpoint/arrivals
	/// Used of you end the round at centcom.
	var/datum/spawnpoint/spawnpoint_left = /datum/spawnpoint/arrivals
	/// Used if you end the round on the station.
	var/datum/spawnpoint/spawnpoint_stayed = /datum/spawnpoint/cryo
...
//! ## Misc
	/// The icon which contains the lobby image(s)
	var/lobby_icon = 'icons/misc/title.dmi'
	/// The list of lobby screen to pick() from. If left unset the first icon state is always selected.
	var/list/lobby_screens = list("mockingjay00")

	/// The default lawset use by synth units, if not overriden by their laws var.
	var/default_law_type = /datum/ai_laws/nanotrasen
	...
	/// Types of `/datum/planet`s that will be instantiated by SSPlanets.
	var/list/planet_datums_to_make = list()
 ...
 */
/datum/map/proc/get_map_levels(srcz, long_range = TRUE, om_range = 0)
	// Overmap behavior
	if(use_overmap)
...
/**
 * Access check is of the type requires one.
 * These have been carefully selected to avoid allowing the janitor to see channels he shouldn't.
...
/datum/map_z_level
	/// Actual z-index of the zlevel. This had better be right!
	var/z = 0
	/// Friendly name of the zlevel.
	var/name
	/// Bitflag of which *_levels lists this z should be put into.
	var/flags = NONE
	/// Type path of the base turf for this z.
	var/turf/base_turf = /turf/space
	/// Percentile chance this z will be chosen for map-edge space transit.
	var/transit_chance = 0

//! ## Holomaps
	/// Number of pixels to offset the map right (for centering) for this z.
	var/holomap_offset_x = -1
	/// Number of pixels to offset the map up (for centering) for this z.
	var/holomap_offset_y = -1
	/// x position of the holomap legend for this z.
	var/holomap_legend_x = 96
	/// y position of the holomap legend for this z.
